// assets/js/private-tours-data.js
const privateToursData = [
    {
        id: 'vnc404',
        title: 'VNC404 Da Nang - Hue - Hoi An (Da Nang 3 nights)',
        region: 'central',
        category: 'comprehensive',
        excerpt: 'Comprehensive 4-day Central Vietnam tour staying 3 nights in Da Nang, visiting Hue Imperial City, Hoi An Ancient Town, and Ba Na Hills with Golden Hand Bridge.',
        description: 'Experience the best of Central Vietnam with this 4-day comprehensive tour featuring 3 nights accommodation in Da Nang, visits to UNESCO World Heritage sites in Hue and Hoi An, plus the spectacular Ba Na Hills with Golden Hand Bridge.',
        image: {
            url: 'https://vn-to-you-tour.vercel.app/assets/images/tours/vnc404.jpg',
            alt: 'VNC404 Da Nang Hue Hoi An Tour'
        },
        duration: {
            display: '4 Days / 3 Nights',
            days: 4,
            nights: 3
        },
        price: {
            display: 'Please contact for pricing',
            amount: 0,
            currency: 'USD'
        },
        groupSize: 'Private tour for 2-10 people',
        highlights: [
            'Seafood hotpot featuring fresh Da Nang seafood and half a lobster per guest',
            'Buffet at Ba Na Hills with Golden Hand Bridge visit',
            'All 3 nights accommodation in Da Nang for convenience',
            'Explore Hoi An Ancient Town with Japanese Covered Bridge',
            'Full day Hue Imperial City tour with Thien Mu Pagoda',
            'Visit Incense Village and cyclo tour in Hue',
            'Multiple hotel options from 3* to 5* in Da Nang',
            'Ba Na Hills Fantasy Park and French Village experience'
        ],
        itinerary: [
            {
                day: 1,
                title: 'Airport Pickup - Hoi An Ancient Town - Da Nang',
                meals: 'Lunch, Dinner',
                activities: [
                    'Pick up at airport or hotel',
                    'Transfer to Hoi An',
                    'Explore Hoi An Ancient Town',
                    'Visit Japanese Covered Bridge',
                    'Tour Tan Ky Old House',
                    'Visit Assembly Hall (Hoi Quan Phuoc Kien)',
                    'Return to Da Nang for overnight'
                ],
                accommodation: 'Da Nang Hotel (3*/4*/5* options available)'
            },
            {
                day: 2,
                title: 'Full Day Ba Na Hills Adventure',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Full day visit to Ba Na Hills',
                    'Transfer to Ba Na Hills via cable car',
                    'Visit French Village and Moon Castle',
                    'Explore Fantasy Park amusement area',
                    'Visit Golden Hand Bridge',
                    'Explore Le Jardin D Amour Flower Garden',
                    'Buffet lunch at Ba Na Hills',
                    'Return to Da Nang for overnight'
                ],
                accommodation: 'Da Nang Hotel (3*/4*/5* options available)',
                notes: 'Optional activities: Wax Museum ($4 USD/pax), Alpine Coaster ($4 USD/pax), Wine Cellar ($4 USD/pax)'
            },
            {
                day: 3,
                title: 'Full Day Hue Imperial City Tour',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Full day visit to Hue',
                    'Visit Incense Village',
                    'Explore Thien Mu Pagoda',
                    'Visit Hue Imperial City (outside) by cyclo',
                    'Return to Da Nang for overnight'
                ],
                accommodation: 'Da Nang Hotel (3*/4*/5* options available)'
            },
            {
                day: 4,
                title: 'Da Nang City Tour - Departure',
                meals: 'Breakfast',
                activities: [
                    'Breakfast at hotel',
                    'Visit Linh Ung Pagoda',
                    'Coffee break at Son Tra Marina (coffee fee not included)',
                    'Visit Dragon Bridge',
                    'Explore Love Bridge',
                    'Shopping at Han Market or Big C',
                    'End of tour - airport transfer'
                ],
                accommodation: 'None'
            }
        ],
        accommodations: {
            danang: {
                '3star': [
                    'Sepon Blue Hotel 3*',
                    'Pandora Danang Hotel',
                    'Or similar'
                ],
                '4star': [
                    'Luxtery Da Nang 4*',
                    'Nhu Minh Danang 4*',
                    'Or similar'
                ],
                '5star': [
                    'Golden Bay Danang Hotel 5*',
                    'Muong Thanh Luxury Danang 5*',
                    'Or similar'
                ]
            },
            hue: {
                '3star': [
                    'Duy Tan Hue Hotel 3*',
                    'Thanh Lich Hotel 3*',
                    'Or similar'
                ],
                '4star': [
                    'Thanh Lich Royal Boutique 4*',
                    'Romance Hotel Hue 4*',
                    'Asia Hue Hotel 4*',
                    'Or similar'
                ],
                '5star': [
                    'To be confirmed'
                ]
            }
        },
        included: [
            'Accommodation twin share room (3 nights in Da Nang)',
            'All meals as specified: (B) Breakfast, (L) Lunch, (D) Dinner',
            'Transportation with air-conditioning',
            'English/Chinese/Thai speaking guide',
            'All entrance fees',
            'Drinking water'
        ],
        excluded: [
            'Visa stamping fee in Vietnam',
            'Personal travel insurance',
            'Any flights to and from Vietnam',
            'All personal expenses',
            'All gratuities and tips to driver and tour guide',
            'Early check in & late check out at hotels',
            'Bank fees',
            'Surcharge on Christmas or New Year time',
            'Tips: Guide $2-4 USD/day, Driver $1-2 USD/day (depending on group size)'
        ],
        termsAndConditions: [
            'Rates not applicable during peak seasons: New Year (28 Dec - 2 Jan), Vietnamese New Year (25 Jan - 2 Feb), Apr 5-7, Apr 30 - May 2, Sep 1-3, Dec 23-25',
            'No refund for unused services except in case of storm or flood',
            'Cancellations 7 days before departure: no refund if deposits paid',
            'Company not responsible for expenses due to force majeure',
            'Itinerary may be changed without prior notice with customer interests prioritized'
        ],
        tags: ['central-vietnam', 'ba-na-hills', 'hoi-an', 'hue', 'comprehensive', 'cultural-heritage', 'golden-bridge', 'imperial-city'],
        featured: false,
        status: 'active',
        lastUpdated: '2025-11-14',
        seoTitle: 'VNC404 Da Nang Hue Hoi An 4D3N Private Tour | VN To You',
        seoDescription: 'Experience Central Vietnam with our comprehensive 4-day tour staying in Da Nang, visiting Hue Imperial City, Hoi An Ancient Town, and Ba Na Hills Golden Hand Bridge.',
        seoKeywords: 'Da Nang tour, Hue Imperial City, Hoi An ancient town, Ba Na Hills, Central Vietnam tour, private tour Vietnam, Golden Hand Bridge'
    },
    {
        id: 'vnc302',
        title: 'VNC302 Da Nang - Hoi An - Full day visit Ba Na Hills',
        region: 'central',
        category: 'standard',
        excerpt: 'Experience Central Vietnam with 3 days exploring Hoi An Ancient Town, full day at Ba Na Hills with Golden Hand Bridge, and Da Nang city highlights.',
        description: 'Discover the perfect blend of culture and adventure with this 3-day tour featuring Hoi An Ancient Town, a full day exploring Ba Na Hills including Golden Hand Bridge and French Village, plus Da Nang city attractions.',
        image: {
            url: 'https://vn-to-you-tour.vercel.app/assets/images/tours/vnc302.jpg',
            alt: 'VNC302 Da Nang Hoi An Ba Na Hills Tour'
        },
        duration: {
            display: '3 Days / 2 Nights',
            days: 3,
            nights: 2
        },
        price: {
            display: 'Please contact us',
            amount: 0,
            currency: 'USD'
        },
        groupSize: 'Private tour for 2-10 people',
        highlights: [
            'Seafood hotpot featuring fresh Da Nang seafood and half a lobster per guest',
            'Buffet at Ba Na Hills',
            'Full day visit to Ba Na Hills with Golden Hand Bridge',
            'Explore Hoi An Ancient Town, Japanese Covered Bridge, Tan Ky Old House',
            'Visit French Village, Moon Castle, and Fantasy Park',
            'Multiple hotel options from 3* to 5* in Da Nang',
            'Linh Ung Pagoda and Son Tra Peninsula tour'
        ],
        itinerary: [
            {
                day: 1,
                title: 'Airport Pickup - Hoi An Ancient Town - Da Nang',
                meals: 'Lunch, Dinner',
                activities: [
                    'Pick up at airport or hotel',
                    'Transfer to Hoi An',
                    'Explore Hoi An Ancient Town',
                    'Visit Japanese Covered Bridge',
                    'Tour Tan Ky Old House',
                    'Visit Assembly Hall (Hoi Quan Phuoc Kien)',
                    'Return to Da Nang for overnight'
                ],
                accommodation: 'Da Nang Hotel (3*/4*/5* options available)'
            },
            {
                day: 2,
                title: 'Full Day Ba Na Hills Adventure',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Full day visit to Ba Na Hills',
                    'Transfer to Ba Na Hills via cable car',
                    'Visit French Village and Moon Castle',
                    'Explore Fantasy Park amusement area',
                    'Visit Golden Hand Bridge',
                    'Explore Le Jardin D Amour Flower Garden',
                    'Buffet lunch at Ba Na Hills',
                    'Return to Da Nang for overnight'
                ],
                accommodation: 'Da Nang Hotel (3*/4*/5* options available)',
                notes: 'Optional activities: Wax Museum ($4 USD/pax), Alpine Coaster ($4 USD/pax), Wine Cellar ($4 USD/pax)'
            },
            {
                day: 3,
                title: 'Da Nang City Tour - Departure',
                meals: 'Breakfast, Lunch',
                activities: [
                    'Breakfast at hotel',
                    'Visit Linh Ung Pagoda',
                    'Coffee break at Son Tra Marina (coffee fee not included)',
                    'Lunch in Da Nang',
                    'Visit Dragon Bridge',
                    'Explore Love Bridge',
                    'Shopping at Han Market or Big C',
                    'End of tour - airport transfer'
                ],
                accommodation: 'None'
            }
        ],
        accommodations: {
            danang: {
                '3star': [
                    'Sepon Blue Hotel 3*',
                    'Pandora Danang Hotel',
                    'Or similar'
                ],
                '4star': [
                    'Luxtery Da Nang 4*',
                    'Nhu Minh Danang 4*',
                    'Or similar'
                ],
                '5star': [
                    'Golden Bay Danang Hotel 5*',
                    'Muong Thanh Luxury Danang 5*',
                    'Or similar'
                ]
            }
        },
        included: [
            'Accommodation twin share room',
            'All meals as specified: (B) Breakfast, (L) Lunch, (D) Dinner',
            'Transportation with air-conditioning',
            'English/Chinese/Thai speaking guide',
            'All entrance fees',
            'Drinking water'
        ],
        excluded: [
            'Visa stamping fee in Vietnam',
            'Personal travel insurance',
            'Any flights to and from Vietnam',
            'All personal expenses',
            'All gratuities and tips to driver and tour guide',
            'Early check in & late check out at hotels',
            'Bank fees',
            'Surcharge on Christmas or New Year time',
            'Tips: Guide $2-4 USD/day, Driver $1-2 USD/day (depending on group size)'
        ],
        termsAndConditions: [
            'Rates not applicable during peak seasons: New Year (28 Dec - 2 Jan), Vietnamese New Year (25 Jan - 2 Feb), Apr 5-7, Apr 30 - May 2, Sep 1-3, Dec 23-25',
            'No refund for unused services except in case of storm or flood',
            'Cancellations 7 days before departure: no refund if deposits paid',
            'Company not responsible for expenses due to force majeure',
            'Itinerary may be changed without prior notice with customer interests prioritized'
        ],
        tags: ['central-vietnam', 'ba-na-hills', 'hoi-an', 'da-nang', 'standard', 'cultural-heritage', 'golden-bridge'],
        featured: false,
        status: 'active',
        lastUpdated: '2025-11-14',
        seoTitle: 'VNC302 Da Nang Hoi An Ba Na Hills 3D2N Private Tour | VN To You',
        seoDescription: 'Experience the best of Central Vietnam with our 3-day private tour featuring Hoi An Ancient Town, full day at Ba Na Hills with Golden Hand Bridge, and Da Nang city highlights.',
        seoKeywords: 'Da Nang tour, Hoi An ancient town, Ba Na Hills day trip, Golden Hand Bridge, Central Vietnam tour, private tour Vietnam'
    },
    {
        id: 'vnn301',
        title: 'VNN301 Ha Noi - Sa Pa 3D2N (Sapa 2 nights)',
        region: 'north',
        category: 'signature',
        excerpt: 'Experience the stunning mountain landscapes of Sapa with our 3-day, 2-night tour featuring Fansipan peak, Cat Cat Village, and signature Salmon Hotpot dinner.',
        description: 'VNN301 Ha Noi - Sa Pa 3D2N (Sapa 2 nights) - Discover the breathtaking beauty of Northern Vietnam with 2 nights in Sapa, featuring visits to Cat Cat Village, Fansipan peak, and authentic mountain experiences.',
        image: {
            url: 'https://vn-to-you-tour.vercel.app/assets/images/tours/vnn301.jpg',
            alt: 'VNN301 Ha Noi - Sa Pa 3D2N'
        },
        duration: {
            display: '3 Days / 2 Nights',
            days: 3,
            nights: 2
        },
        price: {
            display: 'Please contact us for pricing',
            amount: 0,
            currency: 'USD'
        },
        groupSize: 'Private tour - all group sizes',
        highlights: [
            'Signature Sapa Dinner: Enjoy Salmon Hotpot and Sashimi paired with Dalat Wine, Vietnam\'s famous vintage, amidst Sapa\'s cool, misty air',
            'Fansipan Buffet offers a wide array of dishes, highlighting the diverse cuisine of Sapa',
            'Visit Cat Cat Village and explore local culture',
            'Shopping at night market Sapa',
            'Cable car to Fansipan peak with Muong Hoa Train experience'
        ],
        itinerary: [
            {
                day: 1,
                title: 'Hanoi to Sapa - Cat Cat Village',
                meals: 'Lunch, Dinner',
                activities: [
                    'Pick up at airport (or hotel)',
                    'Transfer to Sa Pa',
                    'Visit Cat Cat Village',
                    'Shopping at night market Sapa'
                ],
                accommodation: 'Sapa Hotel (3*/4*/5* options available)'
            },
            {
                day: 2,
                title: 'Fansipan Peak Adventure',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Visit Fansipan (include Muong Hoa Train, Buffet Fansipan, Cable Car ticket)',
                    'Visit Moana Cafe (cafe fee not included)',
                    'Dinner (Salmon hotpot + red wine)'
                ],
                accommodation: 'Sapa Hotel (3*/4*/5* options available)',
                notes: 'Optional Funicular Train Fansipan: $7 USD/pax up, $5 USD/pax down (Ideal for elderly guests who wish to skip the 600-step climb to the peak)'
            },
            {
                day: 3,
                title: 'Sapa to Hanoi - City Tour',
                meals: 'Breakfast, Lunch',
                activities: [
                    'Breakfast at hotel',
                    'Back to Hanoi',
                    'Tran Quoc Pagoda',
                    'Hanoi Old Quarter',
                    'Sword Lake Hanoi',
                    'Shopping',
                    'End of tour'
                ],
                accommodation: 'End of tour'
            }
        ],
        accommodations: {
            sapa: {
                threestar: 'Azure Sapa 3* / Sapa Relax Hotel 3* or similar',
                fourstar: 'Sapa Highland Resort & Spa 4* / Charm Sapa Hotel 4* / Hotel De Sapa 4* or similar',
                fivestar: 'Available upon request'
            }
        },
        included: [
            'Accommodation twin share room',
            'Meals: 3 Breakfasts, 3 Lunches, 2 Dinners',
            'Transportation with air-condition',
            'English / Chinese / Thai speaking guide',
            'Entrance fees',
            'Drinking water'
        ],
        excluded: [
            'Visa stamping fee in Vietnam',
            'Personal travel insurance',
            'Any flights to and from Vietnam',
            'All personal expenses',
            'All gratuities and tips to driver and tour guide',
            'Early check in & late check out at hotels',
            'Bank fee',
            'Surcharge on Christmas or New Year time',
            'Tip for guide 2 USD/day / Driver 1 USD/day (Group <10 pax Tip for guide 4 USD/day / Driver 2 USD/day)'
        ],
        termsAndConditions: [
            'Rates are not applicable during: New Year 28 Dec - 2 Jan 25 | Vietnamese New Year 25 Jan - 2 Feb | 5 - 7 Apr | 30 Apr - 2 May | 1 - 3 Sept | 23 - 25 DEC',
            'Tickets no refund in case of the tour does not visit various tourist attractions except in the event of a storm or flood',
            'When there is a force majeure that cannot be solved, the company has the right not to refund in all cases',
            'Cancellations 07 days before departure (or in the case of the company Deposit for hotel, car, etc.) will not return the money',
            'The company has the right not to be responsible for any expenses in the event of force majeure such as cancellation or delay of airlines / natural disasters / riots / political events and lost items at places where beyond the control of the company',
            'The travel program may be changed as appropriate without prior notice. The company will take into account the interests of customers as a priority'
        ],
        tags: ['north-vietnam', 'sapa', 'fansipan', 'mountain-tour', 'cat-cat-village', 'signature-dinner'],
        featured: true,
        status: 'active',
        lastUpdated: '2025-11-14',
        seoTitle: 'VNN301 Ha Noi - Sa Pa 3D2N Private Tour | VN To You',
        seoDescription: 'Experience Sapa\'s stunning mountain beauty with our 3D2N tour featuring Fansipan peak, Cat Cat Village, signature Salmon Hotpot dinner, and comfortable accommodation options.',
        seoKeywords: 'Sapa tour, Fansipan peak, Cat Cat Village, Hanoi to Sapa, mountain tour Vietnam, salmon hotpot Sapa, private tour Sapa'
    },
    {
        id: 'vnn302',
        title: 'VNN302 Ha Noi - Ha Long Bay - Ninh Binh (Ha Long Bay on land)',
        region: 'north',
        category: 'comprehensive',
        excerpt: 'Experience the best of Northern Vietnam with this 3-day tour combining Ha Long Bay cruise, Ninh Binh "Ha Long Bay on land", and Hanoi highlights.',
        description: 'VNN302 Ha Noi - Ha Long Bay - Ninh Binh (Ha Long Bay on land) - A perfect 3-day journey featuring 1 night in Hanoi, 1 night in Halong Bay with fresh seafood dining, and exploring the stunning limestone landscapes of both Ha Long Bay and Ninh Binh.',
        image: {
            url: 'https://vn-to-you-tour.vercel.app/assets/images/tours/vnn302.jpg',
            alt: 'VNN302 Ha Noi - Ha Long Bay - Ninh Binh'
        },
        duration: {
            display: '3 Days / 2 Nights',
            days: 3,
            nights: 2
        },
        price: {
            display: 'Please contact us for pricing',
            amount: 0,
            currency: 'USD'
        },
        groupSize: 'Private tour - all group sizes',
        highlights: [
            'The Sen Buffet, Hanoi\'s largest, features a diverse culinary spread of over 200 Vietnamese and international dishes',
            'A taste of the sea: Fresh Ha Long Bay seafood enjoyed on a characteristic wooden Ha Long Bay vessel',
            'Ha Long Bay boat trip with cave exploration',
            'Tam Coc Boat Trip in Ninh Binh - "Ha Long Bay on land"',
            'Visit historic sites including Dinh Le Tomb and Hoa Lu'
        ],
        itinerary: [
            {
                day: 1,
                title: 'Hanoi to Ha Long Bay',
                meals: 'Lunch, Dinner',
                activities: [
                    'Pick up at airport (or hotel)',
                    'Photo stop at Mega Grand World',
                    'Transfer to Ha Long Bay',
                    'A boat trip through Ha Long Bay, visiting Thien Cung Cave, Dinh Huong Islet, and Cock Fighting Islet',
                    'Dinner with fresh Ha Long Bay seafood',
                    'Night market Halong'
                ],
                accommodation: 'Ha Long Bay Hotel (3*/4*/5* options available)'
            },
            {
                day: 2,
                title: 'Ha Long Bay to Ninh Binh',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Transfer to Ninh Binh',
                    'Tam Coc Boat Trip (Ha Long Bay on land)',
                    'Dinh Le Tomb visit',
                    'Hoa Lu walking street exploration'
                ],
                accommodation: 'Ninh Binh Hotel (3*/4*/5* options available)'
            },
            {
                day: 3,
                title: 'Ninh Binh to Hanoi - City Tour',
                meals: 'Breakfast, Lunch',
                activities: [
                    'Breakfast at hotel',
                    'Back to Hanoi',
                    'Quang Phu Cau Incense Village',
                    'Hanoi Old Quarter',
                    'Sword Lake',
                    'Shopping',
                    'End of tour'
                ],
                accommodation: 'End of tour'
            }
        ],
        accommodations: {
            hanoi: {
                threestar: 'CWD Hanoi Hotel 3*, Quang Ba Trade Union Hotel 3* or similar',
                fourstar: 'Thang Long Opera Hotel 4* / Muong Thanh Central Hotel 4* / Super Candle Hanoi Hotel 4* / Lacasa Hanoi Hotel 4* or similar',
                fivestar: 'Available upon request'
            },
            halong: {
                threestar: 'New Star Halong 3*, Song Loc Luxury Hotel 3* or similar',
                fourstar: 'Marina Halong Hotel 4*, Song Loc Halong Hotel 4* or similar',
                fivestar: 'Available upon request'
            }
        },
        included: [
            'Accommodation twin share room',
            'Meals: 3 Breakfasts, 3 Lunches, 2 Dinners',
            'Transportation with air-condition',
            'English / Chinese / Thai speaking guide',
            'Entrance fees',
            'Drinking water'
        ],
        excluded: [
            'Visa stamping fee in Vietnam',
            'Personal travel insurance',
            'Any flights to and from Vietnam',
            'All personal expenses',
            'All gratuities and tips to driver and tour guide',
            'Early check in & late check out at hotels',
            'Bank fee',
            'Surcharge on Christmas or New Year time',
            'Tip for guide 2 USD/day / Driver 1 USD/day (Group <10 pax Tip for guide 4 USD/day / Driver 2 USD/day)'
        ],
        termsAndConditions: [
            'Rates are not applicable during: New Year 28 Dec - 2 Jan 25 | Vietnamese New Year 25 Jan - 2 Feb | 5 - 7 Apr | 30 Apr - 2 May | 1 - 3 Sept | 23 - 25 DEC',
            'Tickets no refund in case of the tour does not visit various tourist attractions except in the event of a storm or flood',
            'When there is a force majeure that cannot be solved, the company has the right not to refund in all cases',
            'Cancellations 07 days before departure (or in the case of the company Deposit for hotel, car, etc.) will not return the money',
            'The company has the right not to be responsible for any expenses in the event of force majeure such as cancellation or delay of airlines / natural disasters / riots / political events and lost items at places where beyond the control of the company',
            'The travel program may be changed as appropriate without prior notice. The company will take into account the interests of customers as a priority'
        ],
        tags: ['north-vietnam', 'halong-bay', 'ninh-binh', 'hanoi', 'boat-trip', 'seafood', 'comprehensive-tour'],
        featured: true,
        status: 'active',
        lastUpdated: '2025-11-14',
        seoTitle: 'VNN302 Ha Noi - Ha Long Bay - Ninh Binh 3D2N Tour | VN To You',
        seoDescription: 'Explore Northern Vietnam with our comprehensive 3D2N tour featuring Ha Long Bay cruise, Ninh Binh boat trip, fresh seafood dining, and Hanoi city highlights.',
        seoKeywords: 'Ha Long Bay tour, Ninh Binh tour, Hanoi tour, Ha Long Bay on land, Tam Coc boat trip, Northern Vietnam tour, private tour Vietnam'
    },
    {
        id: 'vnn401',
        title: 'VNN401 Sa Pa - Ha Noi city tour',
        region: 'north',
        category: 'comprehensive',
        excerpt: 'Extended 4-day tour combining the mountain beauty of Sapa with comprehensive Hanoi city exploration, featuring 2 nights in Sapa and 1 night in Hanoi.',
        description: 'VNN401 Sa Pa - Ha Noi city tour - Experience the best of Northern Vietnam with 2 nights in Sapa and 1 night in Hanoi. Enjoy signature Sapa experiences including Fansipan peak, Cat Cat Village, and authentic mountain cuisine, followed by comprehensive Hanoi city highlights.',
        image: {
            url: 'https://vn-to-you-tour.vercel.app/assets/images/tours/vnn401.jpg',
            alt: 'VNN401 Sa Pa - Ha Noi city tour'
        },
        duration: {
            display: '4 Days / 3 Nights',
            days: 4,
            nights: 3
        },
        price: {
            display: 'Please contact us for pricing',
            amount: 0,
            currency: 'USD'
        },
        groupSize: 'Private tour - all group sizes',
        highlights: [
            'Signature Sapa Dinner: Enjoy Salmon Hotpot and Sashimi paired with Dalat Wine, Vietnam\'s famous vintage, amidst Sapa\'s cool, misty air',
            'Fansipan Buffet offers a wide array of dishes, highlighting the diverse cuisine of Sapa',
            'The Sen Buffet, Hanoi\'s largest, features a diverse culinary spread of over 200 Vietnamese and international dishes',
            'Visit Vietnam-China Border Gate for unique photo opportunities',
            'Comprehensive Hanoi city tour including HCM Mausoleum and cultural sites'
        ],
        itinerary: [
            {
                day: 1,
                title: 'Arrival to Sapa - Cat Cat Village',
                meals: 'Lunch, Dinner',
                activities: [
                    'Pick up at airport (or hotel)',
                    'Transfer to Sa Pa',
                    'Visit Cat Cat Village',
                    'Shopping at night market Sapa'
                ],
                accommodation: 'Sapa Hotel (3*/4*/5* options available)'
            },
            {
                day: 2,
                title: 'Fansipan Peak Adventure',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Visit Fansipan (include Muong Hoa Train, Buffet Fansipan, Cable Car ticket)',
                    'Visit Moana Cafe (cafe fee not included)',
                    'Dinner (Salmon hotpot + red wine)'
                ],
                accommodation: 'Sapa Hotel (3*/4*/5* options available)',
                notes: 'Optional Funicular Train Fansipan: $7 USD/pax up, $5 USD/pax down (Ideal for elderly guests who wish to skip the 600-step climb to the peak)'
            },
            {
                day: 3,
                title: 'Sapa to Hanoi - Border Gate & City Tour',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Back to Hanoi',
                    'Photo stop at Vietnam-China Border Gate',
                    'Visit Tran Quoc Pagoda',
                    'Ha Noi Church',
                    'Hanoi train track café (cafe fee not included)',
                    'Dinner'
                ],
                accommodation: 'Hanoi Hotel (3*/4*/5* options available)'
            },
            {
                day: 4,
                title: 'Hanoi City Highlights - Departure',
                meals: 'Breakfast, Lunch',
                activities: [
                    'Breakfast at hotel',
                    'HCM Mausoleum',
                    'One Pillar Pagoda',
                    'Lunch (Buffet Sen)',
                    'Hanoi Old Quarter',
                    'Sword Lake Hanoi',
                    'Shopping',
                    'End of tour'
                ],
                accommodation: 'End of tour'
            }
        ],
        accommodations: {
            hanoi: {
                threestar: 'CWD Hanoi Hotel 3*, Quang Ba Trade Union Hotel 3* or similar',
                fourstar: 'Thang Long Opera Hotel 4* / Muong Thanh Central Hotel 4* / Super Candle Hanoi Hotel 4* / Lacasa Hanoi Hotel 4* or similar',
                fivestar: 'Available upon request'
            },
            sapa: {
                threestar: 'Azure Sapa 3* / Sapa Relax Hotel 3* or similar',
                fourstar: 'Sapa Highland Resort & Spa 4* / Charm Sapa Hotel 4* / Hotel De Sapa 4* or similar',
                fivestar: 'Available upon request'
            }
        },
        included: [
            'Accommodation twin share room',
            'Meals: 4 Breakfasts, 4 Lunches, 3 Dinners',
            'Transportation with air-condition',
            'English / Chinese / Thai speaking guide',
            'Entrance fees',
            'Drinking water'
        ],
        excluded: [
            'Visa stamping fee in Vietnam',
            'Personal travel insurance',
            'Any flights to and from Vietnam',
            'All personal expenses',
            'All gratuities and tips to driver and tour guide',
            'Early check in & late check out at hotels',
            'Bank fee',
            'Surcharge on Christmas or New Year time',
            'Tip for guide 2 USD/day / Driver 1 USD/day (Group <10 pax Tip for guide 4 USD/day / Driver 2 USD/day)'
        ],
        termsAndConditions: [
            'Rates are not applicable during: New Year 28 Dec - 2 Jan 25 | Vietnamese New Year 25 Jan - 2 Feb | 5 - 7 Apr | 30 Apr - 2 May | 1 - 3 Sept | 23 - 25 DEC',
            'Tickets no refund in case of the tour does not visit various tourist attractions except in the event of a storm or flood',
            'When there is a force majeure that cannot be solved, the company has the right not to refund in all cases',
            'Cancellations 07 days before departure (or in the case of the company Deposit for hotel, car, etc.) will not return the money',
            'The company has the right not to be responsible for any expenses in the event of force majeure such as cancellation or delay of airlines / natural disasters / riots / political events and lost items at places where beyond the control of the company',
            'The travel program may be changed as appropriate without prior notice. The company will take into account the interests of customers as a priority'
        ],
        tags: ['north-vietnam', 'sapa', 'hanoi', 'fansipan', 'comprehensive-tour', 'border-gate', 'city-tour'],
        featured: true,
        status: 'active',
        lastUpdated: '2025-11-14',
        seoTitle: 'VNN401 Sa Pa - Ha Noi City Tour 4D3N | VN To You',
        seoDescription: 'Comprehensive 4-day Northern Vietnam tour combining Sapa mountain experiences with extensive Hanoi city exploration. Features Fansipan peak, Vietnam-China border, and cultural highlights.',
        seoKeywords: 'Sapa Hanoi tour, Fansipan peak, Vietnam China border, comprehensive Hanoi tour, Northern Vietnam 4 days, Salmon hotpot Sapa, HCM Mausoleum'
    },
    {
        id: 'vnn402',
        title: 'VNN402 Sapa - Hanoi city tour - Ninh Binh (Halong Bay on land)',
        region: 'north',
        category: 'comprehensive',
        excerpt: 'Complete Northern Vietnam experience combining Sapa mountain beauty, Hanoi city highlights, and Ninh Binh "Halong Bay on land" in a comprehensive 4-day tour.',
        description: 'VNN402 Sapa - Hanoi city tour - Ninh Binh (Halong Bay on land) - The ultimate Northern Vietnam adventure featuring 2 nights in Sapa, 1 night in Hanoi, plus a day trip to Ninh Binh. Experience mountain landscapes, cultural sites, and limestone karst beauty.',
        image: {
            url: 'https://vn-to-you-tour.vercel.app/assets/images/tours/vnn402.jpg',
            alt: 'VNN402 Sapa - Hanoi - Ninh Binh Tour'
        },
        duration: {
            display: '4 Days / 3 Nights',
            days: 4,
            nights: 3
        },
        price: {
            display: 'Please contact us for pricing',
            amount: 0,
            currency: 'USD'
        },
        groupSize: 'Private tour - all group sizes',
        highlights: [
            'Signature Sapa Dinner: Enjoy Salmon Hotpot and Sashimi paired with Dalat Wine, Vietnam\'s famous vintage, amidst Sapa\'s cool, misty air',
            'Fansipan Buffet offers a wide array of dishes, highlighting the diverse cuisine of Sapa',
            'The Sen Buffet, Hanoi\'s largest, features a diverse culinary spread of over 200 Vietnamese and international dishes',
            'Visit Vietnam-China Border Gate for unique photo opportunities',
            'Tam Coc boat trip - experience "Halong Bay on land" limestone landscapes',
            'Quang Phu Cau Incense Village cultural experience'
        ],
        itinerary: [
            {
                day: 1,
                title: 'Arrival to Sapa - Cat Cat Village',
                meals: 'Lunch, Dinner',
                activities: [
                    'Pick up at airport (or hotel)',
                    'Transfer to Sa Pa',
                    'Visit Cat Cat Village',
                    'Shopping at night market Sapa'
                ],
                accommodation: 'Sapa Hotel (3*/4*/5* options available)'
            },
            {
                day: 2,
                title: 'Fansipan Peak Adventure',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Visit Fansipan (include Muong Hoa Train, Buffet Fansipan, Cable Car ticket)',
                    'Visit Moana Cafe (cafe fee not included)',
                    'Dinner (Salmon hotpot + red wine)'
                ],
                accommodation: 'Sapa Hotel (3*/4*/5* options available)',
                notes: 'Optional Funicular Train Fansipan: $7 USD/pax up, $5 USD/pax down (Ideal for elderly guests who wish to skip the 600-step climb to the peak)'
            },
            {
                day: 3,
                title: 'Sapa to Hanoi - Border Gate & City Tour',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Back to Hanoi',
                    'Photo stop at Vietnam-China Border Gate',
                    'Visit Tran Quoc Pagoda',
                    'Hanoi Old Quarter',
                    'Sword Lake Hanoi',
                    'Ha Noi Church',
                    'Hanoi train track café (cafe fee not included)',
                    'Dinner'
                ],
                accommodation: 'Hanoi Hotel (3*/4*/5* options available)'
            },
            {
                day: 4,
                title: 'Ninh Binh Day Trip - Departure',
                meals: 'Breakfast, Lunch',
                activities: [
                    'Breakfast at hotel',
                    'Transfer to Ninh Binh',
                    'Photo stop at Quang Phu Cau Incense Village',
                    'Boat trip Tam Coc (Halong Bay on land)',
                    'Lunch',
                    'Back to Hanoi',
                    'End of tour'
                ],
                accommodation: 'End of tour'
            }
        ],
        accommodations: {
            hanoi: {
                threestar: 'CWD Hanoi Hotel 3*, Quang Ba Trade Union Hotel 3* or similar',
                fourstar: 'Thang Long Opera Hotel 4* / Muong Thanh Central Hotel 4* / Super Candle Hanoi Hotel 4* / Lacasa Hanoi Hotel 4* or similar',
                fivestar: 'Available upon request'
            },
            sapa: {
                threestar: 'Azure Sapa 3* / Sapa Relax Hotel 3* or similar',
                fourstar: 'Sapa Highland Resort & Spa 4* / Charm Sapa Hotel 4* / Hotel De Sapa 4* or similar',
                fivestar: 'Available upon request'
            }
        },
        included: [
            'Accommodation twin share room',
            'Meals: 4 Breakfasts, 4 Lunches, 3 Dinners',
            'Transportation with air-condition',
            'English / Chinese / Thai speaking guide',
            'Entrance fees',
            'Drinking water'
        ],
        excluded: [
            'Visa stamping fee in Vietnam',
            'Personal travel insurance',
            'Any flights to and from Vietnam',
            'All personal expenses',
            'All gratuities and tips to driver and tour guide',
            'Early check in & late check out at hotels',
            'Bank fee',
            'Surcharge on Christmas or New Year time',
            'Tip for guide 2 USD/day / Driver 1 USD/day (Group <10 pax Tip for guide 4 USD/day / Driver 2 USD/day)'
        ],
        termsAndConditions: [
            'Rates are not applicable during: New Year 28 Dec - 2 Jan 25 | Vietnamese New Year 25 Jan - 2 Feb | 5 - 7 Apr | 30 Apr - 2 May | 1 - 3 Sept | 23 - 25 DEC',
            'Tickets no refund in case of the tour does not visit various tourist attractions except in the event of a storm or flood',
            'When there is a force majeure that cannot be solved, the company has the right not to refund in all cases',
            'Cancellations 07 days before departure (or in the case of the company Deposit for hotel, car, etc.) will not return the money',
            'The company has the right not to be responsible for any expenses in the event of force majeure such as cancellation or delay of airlines / natural disasters / riots / political events and lost items at places where beyond the control of the company',
            'The travel program may be changed as appropriate without prior notice. The company will take into account the interests of customers as a priority'
        ],
        tags: ['north-vietnam', 'sapa', 'hanoi', 'ninh-binh', 'fansipan', 'tam-coc', 'comprehensive-tour', 'border-gate'],
        featured: true,
        status: 'active',
        lastUpdated: '2025-11-14',
        seoTitle: 'VNN402 Sapa - Hanoi - Ninh Binh 4D3N Comprehensive Tour | VN To You',
        seoDescription: 'Ultimate Northern Vietnam tour combining Sapa mountains, Hanoi city highlights, and Ninh Binh "Halong Bay on land". Features Fansipan peak, Vietnam-China border, and Tam Coc boat trip.',
        seoKeywords: 'Sapa Hanoi Ninh Binh tour, Fansipan Tam Coc, Northern Vietnam comprehensive tour, Vietnam China border, Halong Bay on land, 4 day Vietnam tour'
    },
    {
        id: 'vnn404',
        title: 'VNN404 Sapa - Hanoi - Ha Long Bay - Ninh Binh (Ha Long on land)',
        region: 'north',
        category: 'comprehensive',
        excerpt: 'Ultimate Northern Vietnam grand tour covering all major highlights: Sapa mountains, Hanoi city, Ha Long Bay cruise, and Ninh Binh limestone landscapes in 4 amazing days.',
        description: 'VNN404 Sapa - Hanoi - Ha Long Bay - Ninh Binh (Ha Long on land) - The most comprehensive Northern Vietnam experience featuring 1 night each in Sapa, Hanoi, and Ha Long Bay. Discover mountain peaks, cultural heritage, and both Ha Long Bay and its "on land" counterpart in Ninh Binh.',
        image: {
            url: 'https://vn-to-you-tour.vercel.app/assets/images/tours/vnn404.jpg',
            alt: 'VNN404 Sapa - Hanoi - Ha Long Bay - Ninh Binh Tour'
        },
        duration: {
            display: '4 Days / 3 Nights',
            days: 4,
            nights: 3
        },
        price: {
            display: 'Please contact us for pricing',
            amount: 0,
            currency: 'USD'
        },
        groupSize: 'Private tour - all group sizes',
        highlights: [
            'Signature Sapa Dinner: Enjoy Salmon Hotpot and Sashimi paired with Dalat Wine, Vietnam\'s famous vintage, amidst Sapa\'s cool, misty air',
            'Fansipan Buffet offers a wide array of dishes, highlighting the diverse cuisine of Sapa',
            'A taste of the sea: Fresh Ha Long Bay seafood enjoyed on a characteristic wooden Ha Long Bay vessel',
            'Experience both Ha Long Bay and Ninh Binh "Ha Long on land" limestone landscapes',
            'Visit all major Northern Vietnam destinations in one comprehensive tour',
            'Quang Phu Cau Incense Village cultural experience'
        ],
        itinerary: [
            {
                day: 1,
                title: 'Arrival to Sapa - Cat Cat Village',
                meals: 'Lunch, Dinner',
                activities: [
                    'Pick up at airport (or hotel)',
                    'Transfer to Sa Pa',
                    'Visit Cat Cat Village',
                    'Shopping at night market Sapa'
                ],
                accommodation: 'Sapa Hotel (3*/4*/5* options available)'
            },
            {
                day: 2,
                title: 'Fansipan Peak - Return to Hanoi',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Visit Fansipan (include Muong Hoa Train, Buffet Fansipan, Cable Car ticket)',
                    'Visit Moana Cafe (cafe fee not included)',
                    'Dinner (Salmon hotpot + red wine)',
                    'Back to Ha Noi'
                ],
                accommodation: 'Hanoi Hotel (3*/4*/5* options available)',
                notes: 'Optional Funicular Train Fansipan: $7 USD/pax up, $5 USD/pax down (Ideal for elderly guests who wish to skip the 600-step climb to the peak)'
            },
            {
                day: 3,
                title: 'Ninh Binh - Ha Long Bay',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Transfer to Ninh Binh',
                    'Photo stop at Quang Phu Cau Incense Village',
                    'Boat trip Tam Coc (Ha Long on land)',
                    'Transfer to Ha Long Bay',
                    'Dinner with fresh Ha Long Bay seafood',
                    'Halong night market'
                ],
                accommodation: 'Ha Long Bay Hotel (3*/4*/5* options available)'
            },
            {
                day: 4,
                title: 'Ha Long Bay Cruise - Return to Hanoi',
                meals: 'Breakfast, Lunch',
                activities: [
                    'Breakfast at hotel',
                    'A boat trip through Ha Long Bay, visiting Thien Cung Cave, Dinh Huong Islet, and Cock Fighting Islet',
                    'Lunch on boat',
                    'Back to Ha Noi',
                    'Hanoi Old Quarter',
                    'Sword Lake Hanoi',
                    'Shopping',
                    'End of tour'
                ],
                accommodation: 'End of tour'
            }
        ],
        accommodations: {
            hanoi: {
                threestar: 'CWD Hanoi Hotel 3*, Quang Ba Trade Union Hotel 3* or similar',
                fourstar: 'Thang Long Opera Hotel 4* / Muong Thanh Central Hotel 4* / Super Candle Hanoi Hotel 4* / Lacasa Hanoi Hotel 4* or similar',
                fivestar: 'Available upon request'
            },
            sapa: {
                threestar: 'Azure Sapa 3* / Sapa Relax Hotel 3* or similar',
                fourstar: 'Sapa Highland Resort & Spa 4* / Charm Sapa Hotel 4* / Hotel De Sapa 4* or similar',
                fivestar: 'Available upon request'
            },
            halong: {
                threestar: 'New Star Halong 3*, Song Loc Luxury Hotel 3* or similar',
                fourstar: 'Marina Halong Hotel 4*, Song Loc Halong Hotel 4* or similar',
                fivestar: 'Available upon request'
            }
        },
        included: [
            'Accommodation twin share room',
            'Meals: 4 Breakfasts, 4 Lunches, 3 Dinners',
            'Transportation with air-condition',
            'English / Chinese / Thai speaking guide',
            'Entrance fees',
            'Drinking water'
        ],
        excluded: [
            'Visa stamping fee in Vietnam',
            'Personal travel insurance',
            'Any flights to and from Vietnam',
            'All personal expenses',
            'All gratuities and tips to driver and tour guide',
            'Early check in & late check out at hotels',
            'Bank fee',
            'Surcharge on Christmas or New Year time',
            'Tip for guide 2 USD/day / Driver 1 USD/day (Group <10 pax Tip for guide 4 USD/day / Driver 2 USD/day)'
        ],
        termsAndConditions: [
            'Rates are not applicable during: New Year 28 Dec - 2 Jan 25 | Vietnamese New Year 25 Jan - 2 Feb | 5 - 7 Apr | 30 Apr - 2 May | 1 - 3 Sept | 23 - 25 DEC',
            'Tickets no refund in case of the tour does not visit various tourist attractions except in the event of a storm or flood',
            'When there is a force majeure that cannot be solved, the company has the right not to refund in all cases',
            'Cancellations 07 days before departure (or in the case of the company Deposit for hotel, car, etc.) will not return the money',
            'The company has the right not to be responsible for any expenses in the event of force majeure such as cancellation or delay of airlines / natural disasters / riots / political events and lost items at places where beyond the control of the company',
            'The travel program may be changed as appropriate without prior notice. The company will take into account the interests of customers as a priority'
        ],
        tags: ['north-vietnam', 'sapa', 'hanoi', 'halong-bay', 'ninh-binh', 'fansipan', 'tam-coc', 'grand-tour', 'comprehensive'],
        featured: true,
        status: 'active',
        lastUpdated: '2025-11-14',
        seoTitle: 'VNN404 Sapa - Hanoi - Ha Long Bay - Ninh Binh Grand Tour 4D3N | VN To You',
        seoDescription: 'Ultimate Northern Vietnam grand tour featuring Sapa mountains, Hanoi city, Ha Long Bay cruise, and Ninh Binh limestone landscapes. Complete 4-day Northern Vietnam experience with all major highlights.',
        seoKeywords: 'Northern Vietnam grand tour, Sapa Hanoi Halong Ninh Binh, Fansipan Ha Long Bay, comprehensive Vietnam tour, 4 day Vietnam tour, Northern Vietnam highlights'
    },
    {
        id: 'vnn407',
        title: 'VNN407 Ha Noi - Sapa - Ha Long Bay',
        region: 'north',
        category: 'premium',
        excerpt: 'Premium Northern Vietnam tour combining the mountain beauty of Sapa with the marine wonders of Ha Long Bay, featuring 2 nights in Sapa and 1 night in Ha Long Bay.',
        description: 'VNN407 Ha Noi - Sapa - Ha Long Bay - Experience the perfect combination of mountains and sea with 2 nights in Sapa and 1 night in Ha Long Bay. Enjoy signature Sapa dining experiences and fresh Ha Long Bay seafood on traditional wooden vessels.',
        image: {
            url: 'https://vn-to-you-tour.vercel.app/assets/images/tours/vnn407.jpg',
            alt: 'VNN407 Ha Noi - Sapa - Ha Long Bay Tour'
        },
        duration: {
            display: '4 Days / 3 Nights',
            days: 4,
            nights: 3
        },
        price: {
            display: 'Please contact us for pricing',
            amount: 0,
            currency: 'USD'
        },
        groupSize: 'Private tour - all group sizes',
        highlights: [
            'Signature Sapa Dinner: Enjoy Salmon Hotpot and Sashimi paired with Dalat Wine, Vietnam\'s famous vintage, amidst Sapa\'s cool, misty air',
            'Fansipan Buffet offers a wide array of dishes, highlighting the diverse cuisine of Sapa',
            'A taste of the sea: Fresh Ha Long Bay seafood enjoyed on a characteristic wooden Ha Long Bay vessel',
            'Visit Vietnam-China Border Gate for unique photo opportunities',
            'Explore Mega Grand World Ha Noi entertainment complex',
            'Experience both mountain and marine landscapes of Northern Vietnam'
        ],
        itinerary: [
            {
                day: 1,
                title: 'Hanoi to Sapa - Cat Cat Village',
                meals: 'Lunch, Dinner',
                activities: [
                    'Pick up at airport (or hotel)',
                    'Transfer to Sa Pa',
                    'Visit Cat Cat Village',
                    'Shopping at night market Sapa'
                ],
                accommodation: 'Sapa Hotel (3*/4*/5* options available)'
            },
            {
                day: 2,
                title: 'Fansipan Peak Adventure',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Visit Fansipan (include Muong Hoa Train, Buffet Fansipan, Cable Car ticket)',
                    'Visit Moana Cafe (cafe fee not included)',
                    'Dinner (Salmon hotpot + red wine)'
                ],
                accommodation: 'Sapa Hotel (3*/4*/5* options available)',
                notes: 'Optional Funicular Train Fansipan: $7 USD/pax up, $5 USD/pax down (Ideal for elderly guests who wish to skip the 600-step climb to the peak)'
            },
            {
                day: 3,
                title: 'Sapa to Ha Long Bay via Hanoi',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Back to Hanoi',
                    'Photo stop at Vietnam-China Border Gate',
                    'Tran Quoc Pagoda',
                    'Mega Grand World Ha Noi',
                    'Dinner with fresh Ha Long Bay seafood',
                    'Ha Long night market'
                ],
                accommodation: 'Ha Long Bay Hotel (3*/4*/5* options available)'
            },
            {
                day: 4,
                title: 'Ha Long Bay Cruise - Return to Hanoi',
                meals: 'Breakfast, Lunch',
                activities: [
                    'A boat trip through Ha Long Bay, visiting Thien Cung Cave, Dinh Huong Islet, and Cock Fighting Islet',
                    'Lunch on boat with fresh seafood',
                    'Back to Ha Noi',
                    'Hanoi Old Quarter',
                    'Sword Lake Hanoi',
                    'Shopping',
                    'End of tour'
                ],
                accommodation: 'End of tour'
            }
        ],
        accommodations: {
            sapa: {
                threestar: 'Azure Sapa 3* / Sapa Relax Hotel 3* or similar',
                fourstar: 'Sapa Highland Resort & Spa 4* / Charm Sapa Hotel 4* / Hotel De Sapa 4* or similar',
                fivestar: 'Available upon request'
            },
            halong: {
                threestar: 'New Star Halong 3*, Song Loc Luxury Hotel 3* or similar',
                fourstar: 'Marina Halong Hotel 4*, Song Loc Halong Hotel 4* or similar',
                fivestar: 'Available upon request'
            }
        },
        included: [
            'Accommodation twin share room',
            'Meals: 4 Breakfasts, 4 Lunches, 3 Dinners',
            'Transportation with air-condition',
            'English / Chinese / Thai speaking guide',
            'Entrance fees',
            'Drinking water'
        ],
        excluded: [
            'Visa stamping fee in Vietnam',
            'Personal travel insurance',
            'Any flights to and from Vietnam',
            'All personal expenses',
            'All gratuities and tips to driver and tour guide',
            'Early check in & late check out at hotels',
            'Bank fee',
            'Surcharge on Christmas or New Year time',
            'Tip for guide 2 USD/day / Driver 1 USD/day (Group <10 pax Tip for guide 4 USD/day / Driver 2 USD/day)'
        ],
        termsAndConditions: [
            'Rates are not applicable during: New Year 28 Dec - 2 Jan 25 | Vietnamese New Year 25 Jan - 2 Feb | 5 - 7 Apr | 30 Apr - 2 May | 1 - 3 Sept | 23 - 25 DEC',
            'Tickets no refund in case of the tour does not visit various tourist attractions except in the event of a storm or flood',
            'When there is a force majeure that cannot be solved, the company has the right not to refund in all cases',
            'Cancellations 07 days before departure (or in the case of the company Deposit for hotel, car, etc.) will not return the money',
            'The company has the right not to be responsible for any expenses in the event of force majeure such as cancellation or delay of airlines / natural disasters / riots / political events and lost items at places where beyond the control of the company',
            'The travel program may be changed as appropriate without prior notice. The company will take into account the interests of customers as a priority'
        ],
        tags: ['north-vietnam', 'sapa', 'halong-bay', 'fansipan', 'premium-tour', 'mountain-sea', 'border-gate'],
        featured: true,
        status: 'active',
        lastUpdated: '2025-11-14',
        seoTitle: 'VNN407 Ha Noi - Sapa - Ha Long Bay Premium Tour 4D3N | VN To You',
        seoDescription: 'Premium Northern Vietnam tour combining Sapa mountain beauty with Ha Long Bay marine wonders. Features 2 nights in Sapa, Fansipan peak, Vietnam-China border, and Ha Long Bay cruise with fresh seafood.',
        seoKeywords: 'Sapa Ha Long Bay tour, Fansipan Ha Long cruise, Northern Vietnam premium tour, mountain sea combination, Vietnam China border, Salmon hotpot Sapa'
    },
    {
        id: 'vnc401',
        title: 'VNC401 Da Nang - Ba Na Hills (Over night Ba Na Hills)',
        region: 'central',
        category: 'luxury',
        excerpt: 'Experience 4 days exploring Da Nang, Ba Na Hills, Hoi An, and Hue with overnight stay at Ba Na Hills featuring fresh seafood hotpot and luxury accommodations.',
        description: 'Discover the heart of Central Vietnam with this comprehensive 4-day tour featuring overnight accommodation at Ba Na Hills, Golden Hand Bridge, historic Hoi An ancient town, and the imperial city of Hue.',
        image: {
            url: 'https://vn-to-you-tour.vercel.app/assets/images/tours/vnc401.jpg',
            alt: 'VNC401 Da Nang Ba Na Hills Hoi An Hue Tour'
        },
        duration: {
            display: '4 Days / 3 Nights',
            days: 4,
            nights: 3
        },
        price: {
            display: 'Please contact for pricing',
            amount: 0,
            currency: 'USD'
        },
        groupSize: 'Private tour for 2-10 people',
        highlights: [
            'Overnight stay at Mercure Danang French Village Bana Hills - World Luxury Hotel Awards recognized',
            'Seafood hotpot featuring fresh Da Nang seafood and half a lobster per guest',
            'Buffet at Ba Na Hills',
            'Visit Golden Hand Bridge and Le Jardin D Amour Flower Garden',
            'Explore Hoi An Ancient Town, Japanese Covered Bridge, Tan Ky Old House',
            'Multiple hotel options from 3* to 5* in Da Nang and Hue',
            'Visit Dragon Bridge, Love Bridge, and Han Market in Da Nang'
        ],
        itinerary: [
            {
                day: 1,
                title: 'Airport Pickup - Ba Na Hills - Overnight Ba Na Hills',
                meals: 'Lunch, Dinner',
                activities: [
                    'Pick up at airport or hotel',
                    'Lunch in Da Nang',
                    'Transfer to Ba Na Hills',
                    'Visit French Village and Moon Castle',
                    'Explore Fantasy Park',
                    'Check-in at Mercure Banahills',
                    'Dinner and overnight at Ba Na Hills'
                ],
                accommodation: 'Mercure Banahills',
                notes: 'Optional activities: Wax Museum ($4 USD/pax), Alpine Coaster ($4 USD/pax), Wine Cellar ($4 USD/pax)'
            },
            {
                day: 2,
                title: 'Golden Hand Bridge - Return to Da Nang',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Visit Golden Hand Bridge',
                    'Explore Le Jardin D Amour Flower Garden',
                    '10:00 Check out and return to Da Nang',
                    'Lunch in Da Nang',
                    'Visit Linh Ung Pagoda',
                    'Coffee break at Son Tra Marina (coffee fee not included)',
                    'Return to Da Nang for overnight'
                ],
                accommodation: 'Da Nang Hotel (3*/4*/5* options available)'
            },
            {
                day: 3,
                title: 'Full Day Hoi An Ancient Town',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Transfer to Hoi An',
                    'Explore Hoi An Ancient Town',
                    'Visit Japanese Covered Bridge',
                    'Tour Tan Ky Old House',
                    'Visit Assembly Hall (Hoi Quan Phuoc Kien)',
                    'Return to Da Nang for overnight'
                ],
                accommodation: 'Da Nang Hotel (3*/4*/5* options available)'
            },
            {
                day: 4,
                title: 'Da Nang City Tour - Departure',
                meals: 'Breakfast',
                activities: [
                    'Breakfast at hotel',
                    'Visit Dragon Bridge',
                    'Explore Love Bridge',
                    'Shopping at Han Market',
                    'End of tour - airport transfer'
                ],
                accommodation: 'None'
            }
        ],
        accommodations: {
            banaHills: {
                hotel: 'Mercure Banahills',
                description: 'Luxury accommodation at Ba Na Hills'
            },
            danang: {
                '3star': [
                    'Sepon Blue Hotel 3*',
                    'Pandora Danang Hotel',
                    'Or similar'
                ],
                '4star': [
                    'Luxtery Da Nang 4*',
                    'Nhu Minh Danang 4*',
                    'Or similar'
                ],
                '5star': [
                    'Golden Bay Danang Hotel 5*',
                    'Muong Thanh Luxury Danang 5*',
                    'Or similar'
                ]
            },
            hue: {
                '3star': [
                    'Duy Tan Hue Hotel 3*',
                    'Thanh Lich Hotel 3*',
                    'Or similar'
                ],
                '4star': [
                    'Thanh Lich Royal Boutique 4*',
                    'Romance Hotel Hue 4*',
                    'Asia Hue Hotel 4*',
                    'Or similar'
                ],
                '5star': [
                    'To be confirmed'
                ]
            }
        },
        included: [
            'Accommodation twin share room',
            'All meals as specified: (B) Breakfast, (L) Lunch, (D) Dinner',
            'Transportation with air-conditioning',
            'English/Chinese/Thai speaking guide',
            'All entrance fees',
            'Drinking water'
        ],
        excluded: [
            'Visa stamping fee in Vietnam',
            'Personal travel insurance',
            'Any flights to and from Vietnam',
            'All personal expenses',
            'All gratuities and tips to driver and tour guide',
            'Early check in & late check out at hotels',
            'Bank fees',
            'Surcharge on Christmas or New Year time',
            'Tips: Guide $2-4 USD/day, Driver $1-2 USD/day (depending on group size)'
        ],
        termsAndConditions: [
            'Rates not applicable during peak seasons: New Year (28 Dec - 2 Jan), Vietnamese New Year (25 Jan - 2 Feb), Apr 5-7, Apr 30 - May 2, Sep 1-3, Dec 23-25',
            'No refund for unused services except in case of storm or flood',
            'Cancellations 7 days before departure: no refund if deposits paid',
            'Company not responsible for expenses due to force majeure',
            'Itinerary may be changed without prior notice with customer interests prioritized'
        ],
        tags: ['central-vietnam', 'ba-na-hills', 'hoi-an', 'hue', 'luxury', 'overnight-banahills', 'cultural-heritage', 'golden-bridge'],
        featured: false,
        status: 'active',
        lastUpdated: '2025-11-14',
        seoTitle: 'VNC401 Da Nang Ba Na Hills Hoi An Hue 4D3N Private Tour | VN To You',
        seoDescription: 'Experience the best of Central Vietnam with our 4-day private tour featuring overnight stay at Ba Na Hills, Golden Hand Bridge, Hoi An Ancient Town, and imperial Hue city.',
        seoKeywords: 'Da Nang tour, Ba Na Hills overnight, Hoi An ancient town, Hue imperial city, Central Vietnam tour, Golden Hand Bridge, private tour Vietnam'
    },
    {
        id: 'vnc301',
        title: 'VNC301 Danang - Hoian - O/N Ba Na Hills',
        region: 'central',
        category: 'luxury',
        excerpt: 'Experience the best of Central Vietnam with overnight stay at Ba Na Hills, featuring Golden Hand Bridge, Hoi An Ancient Town, and luxury accommodations.',
        description: 'Discover the magical blend of French colonial architecture and Vietnamese culture with this exclusive 3-day tour featuring overnight accommodation at the stunning Ba Na Hills, exploration of historic Hoi An, and the best of Da Nang.',
        image: {
            url: 'https://vn-to-you-tour.vercel.app/assets/images/tours/vnc301.jpg',
            alt: 'VNC301 Danang Hoian Ba Na Hills Tour'
        },
        duration: {
            display: '3 Days / 2 Nights',
            days: 3,
            nights: 2
        },
        price: {
            display: 'Please contact for pricing',
            amount: 0,
            currency: 'USD'
        },
        groupSize: 'Private tour for 2-10 people',
        highlights: [
            'Overnight stay at Mercure Danang French Village Bana Hills - World Luxury Hotel Awards recognized',
            'Visit the famous Golden Hand Bridge and French Village at Ba Na Hills',
            'Seafood hotpot featuring fresh Da Nang seafood and half a lobster per guest',
            'Explore Hoi An Ancient Town, Japanese Covered Bridge, and Tan Ky Old House',
            'Multiple hotel options from 3* to 5* luxury accommodations in Da Nang',
            'All meals included (3 lunches, 3 dinners) with buffet at Ba Na Hills'
        ],
        itinerary: [
            {
                day: 1,
                title: 'Airport Pickup - Ba Na Hills - French Village',
                meals: 'Lunch, Dinner',
                activities: [
                    'Pick up at airport or hotel',
                    'Welcome lunch in Da Nang',
                    'Transfer to Ba Na Hills via cable car',
                    'Check-in at Mercure Danang French Village Bana Hills',
                    'Visit French Village and Moon Castle',
                    'Explore Fantasy Park amusement area',
                    'Dinner and overnight at Ba Na Hills'
                ],
                accommodation: 'Mercure Danang French Village Bana Hills',
                notes: 'Optional activities: Wax Museum ($4 USD/pax), Alpine Coaster ($4 USD/pax), Wine Cellar ($4 USD/pax)'
            },
            {
                day: 2,
                title: 'Golden Hand Bridge - Hoi An Ancient Town',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel with mountain views',
                    'Visit the world-famous Golden Hand Bridge',
                    'Explore Le Jardin D\'Amour Flower Garden',
                    '10:00 AM Check out and descend to Da Nang',
                    'Lunch featuring fresh seafood hotpot with lobster',
                    'Transfer to Hoi An (45-minute drive)',
                    'Guided tour of Hoi An Ancient Town',
                    'Visit Japanese Covered Bridge and Tan Ky Old House',
                    'Explore Assembly Hall (Hoi Quan Phuoc Kien)',
                    'Return to Da Nang for dinner and overnight'
                ],
                accommodation: 'Selected hotel in Da Nang (3*/4*/5* options available)'
            },
            {
                day: 3,
                title: 'Da Nang City Tour - Departure',
                meals: 'Breakfast, Lunch',
                activities: [
                    'Breakfast at hotel',
                    'Visit Linh Ung Pagoda with panoramic city views',
                    'Coffee break at Son Tra Marina (coffee fee not included)',
                    'Photo stop at famous Dragon Bridge',
                    'Visit Love Bridge (Cau Tinh Yeu)',
                    'Shopping time at Han Market or Big C supermarket',
                    'Farewell lunch',
                    'Airport transfer or hotel drop-off'
                ],
                accommodation: 'End of tour'
            }
        ],
        accommodations: {
            banahills: {
                name: 'Mercure Danang French Village Bana Hills',
                description: 'A perfect imprint of France on the peak of Ba Na, recognized by World Luxury Hotel Awards',
                nights: 1
            },
            danang: {
                options: [
                    {
                        category: '3-Star',
                        hotels: ['Sepon Blue Hotel 3*', 'Pandora Danang Hotel', 'or similar'],
                        nights: 1
                    },
                    {
                        category: '4-Star',
                        hotels: ['Luxtery Da Nang 4*', 'Nhu Minh Danang 4*', 'or similar'],
                        nights: 1
                    },
                    {
                        category: '5-Star',
                        hotels: ['Golden Bay Danang Hotel 5*', 'Muong Thanh Luxury Danang 5*', 'or similar'],
                        nights: 1
                    }
                ]
            }
        },
        included: [
            'Accommodation in twin share rooms',
            'All meals as specified: 2 breakfasts, 3 lunches, 2 dinners',
            'Private transportation with air-conditioning',
            'English/Chinese/Thai speaking guide',
            'All entrance fees to attractions',
            'Drinking water throughout the tour',
            'Cable car tickets to Ba Na Hills',
            'Seafood hotpot with half lobster per guest'
        ],
        excluded: [
            'Visa stamping fee for Vietnam',
            'Personal travel insurance',
            'International flights to and from Vietnam',
            'All personal expenses and shopping',
            'Tips for guide ($2-4 USD/day) and driver ($1-2 USD/day)',
            'Early check-in & late check-out at hotels',
            'Bank transfer fees',
            'Optional activities at Ba Na Hills (Wax Museum, Alpine Coaster, Wine Cellar)',
            'Coffee fee at Son Tra Marina',
            'Christmas/New Year surcharges'
        ],
        termsAndConditions: [
            'Rates not applicable during peak periods: New Year (28 Dec - 2 Jan), Vietnamese New Year (25 Jan - 2 Feb), holidays (5-7 Apr, 30 Apr - 2 May, 1-3 Sept, 23-25 Dec)',
            'No refund for unused services except in cases of natural disasters',
            'Cancellations within 7 days of departure: no refund',
            'Company not responsible for expenses due to force majeure events',
            'Itinerary may be modified for customer benefit without prior notice'
        ],
        bestTime: 'Year-round destination with pleasant weather. Dry season (February-August) is optimal.',
        difficulty: 'Easy to Moderate - Suitable for all ages with some walking involved'
    },
    {
        id: 'vnc402',
        title: 'VNC402 Da Nang - Hoi An - Hue',
        region: 'central',
        category: 'comprehensive',
        excerpt: 'Explore the cultural triangle of Central Vietnam with Da Nang, Hoi An Ancient Town, Ba Na Hills, and the Imperial City of Hue in this 4-day adventure.',
        description: 'Discover the best of Central Vietnam with visits to three UNESCO World Heritage sites, the famous Golden Hand Bridge at Ba Na Hills, and authentic cultural experiences across Da Nang, Hoi An, and Hue.',
        image: {
            url: 'https://vn-to-you-tour.vercel.app/assets/images/tours/vnc402.jpg',
            alt: 'VNC402 Da Nang Hoi An Hue Tour - Golden Hand Bridge and Ancient Architecture'
        },
        duration: {
            display: '4 Days / 3 Nights',
            days: 4,
            nights: 3
        },
        price: {
            display: 'Please contact for pricing',
            amount: 0,
            currency: 'USD'
        },
        groupSize: 'Private tour for 2-10 people',
        highlights: [
            'Overnight stay at Mercure Danang French Village Bana Hills with world-class amenities',
            'Visit the iconic Golden Hand Bridge and French Village at Ba Na Hills',
            'Seafood hotpot featuring fresh Da Nang seafood and half a lobster per guest',
            'Explore Hoi An Ancient Town, Japanese Covered Bridge, and Tan Ky Old House',
            'Discover Hue Imperial City and royal heritage sites',
            'Multiple hotel options from 3* to 5* luxury accommodations in Da Nang and Hue',
            'All meals included with buffet at Ba Na Hills'
        ],
        itinerary: [
            {
                day: 1,
                title: 'Airport Pickup - Da Nang Highlights',
                meals: 'Lunch, Dinner',
                activities: [
                    'Pick up at airport or hotel',
                    'Visit Linh Ung Pagoda with panoramic views',
                    'Coffee break at Son Tra Marina (coffee fee not included)',
                    'Check-in at selected hotel in Da Nang',
                    'Welcome dinner with local specialties',
                    'Overnight in Da Nang'
                ],
                accommodation: 'Selected hotel in Da Nang (3*/4*/5* options available)'
            },
            {
                day: 2,
                title: 'Ba Na Hills - Golden Hand Bridge',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Transfer to Ba Na Hills via cable car',
                    'Visit French Village and Moon Castle',
                    'Explore Fantasy Park amusement area',
                    'Visit the world-famous Golden Hand Bridge',
                    'Explore Le Jardin D\'Amour Flower Garden',
                    'Check-in at Mercure Danang French Village Bana Hills',
                    'Buffet dinner and overnight at Ba Na Hills'
                ],
                accommodation: 'Mercure Danang French Village Bana Hills',
                notes: 'Optional activities: Wax Museum ($4 USD/pax), Alpine Coaster ($4 USD/pax), Wine Cellar ($4 USD/pax)'
            },
            {
                day: 3,
                title: 'Hoi An Ancient Town - Cultural Immersion',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at Ba Na Hills with mountain views',
                    'Descend to Da Nang and transfer to Hoi An',
                    'Guided tour of Hoi An Ancient Town',
                    'Visit Japanese Covered Bridge (400+ years old)',
                    'Explore Tan Ky Old House - traditional merchant house',
                    'Visit Assembly Hall (Hoi Quan Phuoc Kien)',
                    'Free time for shopping and exploring',
                    'Return to Da Nang for dinner and overnight'
                ],
                accommodation: 'Selected hotel in Da Nang'
            },
            {
                day: 4,
                title: 'Da Nang City Tour - Departure',
                meals: 'Breakfast',
                activities: [
                    'Breakfast at hotel',
                    'Visit famous Dragon Bridge',
                    'Photo stop at Love Bridge (Cau Tinh Yeu)',
                    'Shopping time at Han Market or Big C supermarket',
                    'Airport transfer or hotel drop-off',
                    'End of tour'
                ],
                accommodation: 'End of tour'
            }
        ],
        accommodations: {
            banahills: {
                name: 'Mercure Danang French Village Bana Hills',
                description: 'Luxury mountain resort with French colonial architecture, recognized by World Luxury Hotel Awards',
                nights: 1
            },
            danang: {
                options: [
                    {
                        category: '3-Star',
                        hotels: ['Sepon Blue Hotel 3*', 'Pandora Danang Hotel', 'or similar'],
                        nights: 2
                    },
                    {
                        category: '4-Star',
                        hotels: ['Luxtery Da Nang 4*', 'Nhu Minh Danang 4*', 'or similar'],
                        nights: 2
                    },
                    {
                        category: '5-Star',
                        hotels: ['Golden Bay Danang Hotel 5*', 'Muong Thanh Luxury Danang 5*', 'or similar'],
                        nights: 2
                    }
                ]
            },
            hue: {
                options: [
                    {
                        category: '3-Star',
                        hotels: ['Duy Tan Hue Hotel 3*', 'Thanh Lich Hotel 3*', 'or similar'],
                        nights: 0
                    },
                    {
                        category: '4-Star',
                        hotels: ['Thanh Lich Royal Boutique 4*', 'Romance Hotel Hue 4*', 'Asia Hue Hotel 4*', 'or similar'],
                        nights: 0
                    },
                    {
                        category: '5-Star',
                        hotels: ['Available upon request'],
                        nights: 0
                    }
                ]
            }
        },
        included: [
            'Accommodation in twin share rooms (3 nights)',
            'All meals as specified: 3 breakfasts, 3 lunches, 3 dinners',
            'Private transportation with air-conditioning',
            'English/Chinese/Thai speaking guide',
            'All entrance fees to attractions',
            'Drinking water throughout the tour',
            'Cable car tickets to Ba Na Hills',
            'Seafood hotpot with half lobster per guest',
            'Buffet dinner at Ba Na Hills'
        ],
        excluded: [
            'Visa stamping fee for Vietnam',
            'Personal travel insurance',
            'International flights to and from Vietnam',
            'All personal expenses and shopping',
            'Tips for guide ($2-4 USD/day) and driver ($1-2 USD/day)',
            'Early check-in & late check-out at hotels',
            'Bank transfer fees',
            'Optional activities at Ba Na Hills (Wax Museum, Alpine Coaster, Wine Cellar)',
            'Coffee fee at Son Tra Marina',
            'Christmas/New Year surcharges'
        ],
        termsAndConditions: [
            'Rates not applicable during peak periods: New Year (28 Dec - 2 Jan), Vietnamese New Year (25 Jan - 2 Feb), holidays (5-7 Apr, 30 Apr - 2 May, 1-3 Sept, 23-25 Dec)',
            'No refund for unused services except in cases of natural disasters',
            'Cancellations within 7 days of departure: no refund',
            'Company not responsible for expenses due to force majeure events',
            'Itinerary may be modified for customer benefit without prior notice'
        ],
        bestTime: 'Year-round destination with pleasant weather. Dry season (February-August) is optimal.',
        difficulty: 'Easy to Moderate - Suitable for all ages with some walking involved'
    },
    {
        id: 'vns303',
        title: 'VNS303 Phu Quoc Island',
        region: 'south',
        category: 'island',
        excerpt: 'Discover the paradise of Phu Quoc Island with 2 nights stay, featuring fresh seafood, Grand World entertainment complex, Hon Thom Island cable car, and Aquatopia Water Park.',
        description: 'Experience the tropical paradise of Phu Quoc Island with this 3-day tour featuring 2 nights accommodation, delicious fresh seafood meals, traditional Vietnamese cuisine, visits to Grand World entertainment complex, Hon Thom Island cable car adventure, and relaxation at Aquatopia Water Park.',
        image: {
            url: 'https://vn-to-you-tour.vercel.app/assets/images/tours/vns303.jpg',
            alt: 'VNS303 Phu Quoc Island Tour'
        },
        duration: {
            display: '3 Days / 2 Nights',
            days: 3,
            nights: 2
        },
        price: {
            display: 'Please contact us',
            amount: 0,
            currency: 'USD'
        },
        groupSize: 'Private tour for 2-10 people',
        highlights: [
            'A delicious fresh seafood meal in Phu Quoc',
            'One experience of traditional Vietnamese cuisine',
            '2 nights stay on the North Island of Phu Quoc',
            'Grand World Phuquoc - The Sleepless City',
            'Grand World Nightly Light Show',
            'Hon Thom Island cable car adventure',
            'Aquatopia Water Park with buffet included',
            'Sunset Town and Kiss Bridge visit',
            'Photo stop at Dinh Cau Temple',
            'Ho Quoc Temple visit',
            'Multiple hotel options from 3* to 5*'
        ],
        itinerary: [
            {
                day: 1,
                title: 'Airport Pickup - Grand World Phuquoc - Light Show',
                meals: 'Lunch, Dinner',
                activities: [
                    'Pick up at airport (or hotel)',
                    'Transfer to Grand World Phuquoc (The Sleepless City)',
                    'Photo stop at Dinh Cau Temple',
                    'Free time at Grand World Phuquoc',
                    'Grand World Nightly Light Show',
                    'Dinner at local restaurant',
                    '<span style="color: red; font-weight: bold;">Optional Activities at Grand World Phu Quoc:</span>',
                    '<span style="color: red;">• Phu Quoc Teddy Bear Museum Admission: $8 USD/pax</span>',
                    '<span style="color: red;">• Venice River Boat Gondola: $8 USD/pax</span>',
                    '<span style="color: red;">• The Quintessence of Vietnam Show Ticket: $12 USD/pax</span>'
                ],
                accommodation: 'Phu Quoc Hotel'
            },
            {
                day: 2,
                title: 'Hon Thom Island - Aquatopia Water Park - Sunset Town',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Full day visit to Hon Thom Island',
                    'Aquatopia Water Park experience (include cable car, buffet)',
                    'Visit Sunset Town',
                    'Photo session at Kiss Bridge',
                    'Dinner at local restaurant',
                    '<span style="color: red; font-weight: bold;">Optional Shows at Sunset Town:</span>',
                    '<span style="color: red;">• Symphony of the Sea Show (7:45 PM – 8:10 PM): $12 USD/pax</span>',
                    '<span style="color: red;">• Kiss of the Sea main show with fireworks (9:00 PM – 9:30 PM): $45 USD for two people or $35 USD for one person (Note: Not performed on Tuesdays)</span>'
                ],
                accommodation: 'Phu Quoc Hotel'
            },
            {
                day: 3,
                title: 'Ho Quoc Temple - Shopping - Departure',
                meals: 'Breakfast',
                activities: [
                    'Breakfast at hotel',
                    'Visit Ho Quoc Temple',
                    'Shopping time for souvenirs',
                    'End of tour - Transfer to airport or hotel'
                ],
                accommodation: 'End of tour'
            }
        ],
        accommodation: {
            options: [
                {
                    category: '3* Hotel',
                    hotels: ['Standard 3-star accommodations']
                },
                {
                    category: '4* Hotel',
                    hotels: ['VinHolidays Fiesta Phuquoc 4* or similar', 'Wyndham Garden Hotel Phuquoc 4*']
                },
                {
                    category: '5* Hotel',
                    hotels: ['Luxury 5-star accommodations available']
                }
            ]
        },
        meals: {
            total: 'Lunch = 2, Dinner = 2',
            specialties: [
                'Delicious fresh seafood meal in Phu Quoc',
                'Traditional Vietnamese cuisine experience'
            ]
        },
        included: [
            'Accommodation twin share room',
            'Meals: (B) Breakfast, (L) Lunch, (D) Dinner as specified',
            'Transportation with air-conditioning',
            'English/Chinese/Thai speaking guide',
            'Entrance fees to attractions',
            'Drinking water during tours'
        ],
        excluded: [
            'Visa stamping fee in Vietnam',
            'Personal travel insurance',
            'Any flights to and from Vietnam',
            'All personal expenses',
            'All gratuities and tips to driver and tour guide',
            'Early check in & late check out at hotels',
            'Bank fees',
            'Surcharge on Christmas or New Year time',
            'Tips: Guide 2 USD/day, Driver 1 USD/day (Group <10 pax: Guide 4 USD/day, Driver 2 USD/day)'
        ],
        termsAndConditions: [
            'Rates not applicable during peak periods: New Year (28 Dec - 2 Jan), Vietnamese New Year (25 Jan - 2 Feb), holidays (5-7 Apr, 30 Apr - 2 May, 1-3 Sept, 23-25 Dec)',
            'No refund for tickets if tour does not visit attractions except in cases of storms or floods',
            'In cases of force majeure that cannot be resolved, company has right not to refund',
            'Cancellations 7 days before departure (or when company has deposited for hotel, car, etc.) will not be refunded',
            'Company not responsible for expenses due to force majeure events such as flight cancellations/delays, natural disasters, riots, political events, and lost items beyond company control',
            'Travel program may be changed as appropriate without prior notice, considering customer interests as priority'
        ],
        bestTime: 'Year-round tropical destination. Dry season (November-March) offers the best weather conditions.',
        difficulty: 'Easy - Suitable for all ages and fitness levels',
        tags: ['south-vietnam', 'phu-quoc', 'island-tour', 'grand-world', 'hon-thom', 'aquatopia', 'sunset-town'],
        featured: true,
        status: 'active',
        lastUpdated: '2025-11-22',
        seoTitle: 'VNS303 Phu Quoc Island 3D2N Private Tour | VN To You',
        seoDescription: 'Discover Phu Quoc paradise with our 3D2N island tour featuring Grand World entertainment complex, Hon Thom Island cable car, Aquatopia Water Park, and fresh seafood experiences.',
        seoKeywords: 'Phu Quoc tour, Grand World Phuquoc, Hon Thom Island, Aquatopia Water Park, Sunset Town, Kiss Bridge, Phu Quoc island tour, Vietnam island vacation'
    },
    {
        id: 'vns303b',
        title: 'VNS303B Phu Quoc Island (VinWonders & Hon Thom Islet)',
        region: 'south',
        category: 'premium',
        excerpt: 'Experience Phu Quoc\'s two biggest attractions - VinWonders theme park and Hon Thom Islet (Sun World Nature Park) with 2 nights accommodation, featuring fresh seafood and premium entertainment.',
        description: 'VNS303B Phu Quoc Island tour includes the two biggest attractions: VinWonders and Hon Thom Islet (Sun World Nature Park). Enjoy 2 nights on the North Island with delicious fresh seafood meals, traditional Vietnamese cuisine, and world-class entertainment facilities.',
        image: {
            url: 'https://vn-to-you-tour.vercel.app/assets/images/tours/vns303B.jpg',
            alt: 'VNS303B Phu Quoc Island VinWonders Tour'
        },
        duration: {
            display: '3 Days / 2 Nights',
            days: 3,
            nights: 2
        },
        price: {
            display: 'Please contact us',
            amount: 0,
            currency: 'USD'
        },
        groupSize: 'Private tour for 2-10 people',
        highlights: [
            'Two biggest attractions: VinWonders and Hon Thom Islet (Sun World Nature Park)',
            'A delicious fresh seafood meal in Phu Quoc',
            'One experience of traditional Vietnamese cuisine',
            '2 nights stay on the North Island of Phu Quoc',
            'VinWonders Phuquoc full day experience',
            'Hon Thom Island cable car adventure with Aquatopia Water Park',
            'Sunset Town and Kiss Bridge visit',
            'Grand World Phuquoc entertainment complex',
            'Dong Duong night market exploration',
            'Aquarium Phuquoc and Once Show Phuquoc',
            'Multiple hotel options from 3* to 5*'
        ],
        itinerary: [
            {
                day: 1,
                title: 'Airport Pickup - Hon Thom Island - Grand World',
                meals: 'Lunch, Dinner',
                activities: [
                    'Pick up at airport (or hotel)',
                    'Transfer to visit Hon Thom Island',
                    'Aquatopia Water Park experience (include cable car, buffet)',
                    'Visit Sunset Town',
                    'Photo session at Kiss Bridge',
                    'Transfer to Grand World Phuquoc (The Sleepless City)',
                    'Explore Dong Duong night market',
                    'Dinner at local restaurant',
                    '<span style="color: red; font-weight: bold;">Optional Shows at Sunset Town:</span>',
                    '<span style="color: red;">• Symphony of the Sea Show (7:45 PM – 8:10 PM): $12 USD/pax</span>',
                    '<span style="color: red;">• Kiss of the Sea main show with fireworks (9:00 PM – 9:30 PM): $45 USD for two people or $35 USD for one person (Note: Not performed on Tuesdays)</span>'
                ],
                accommodation: 'Phu Quoc Hotel'
            },
            {
                day: 2,
                title: 'VinWonders Phuquoc - Aquarium - Once Show',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Full day visit to VinWonders Phuquoc',
                    'Explore Aquarium Phuquoc',
                    'Experience Once Show Phuquoc',
                    'Dinner at local restaurant',
                    '<span style="color: red; font-weight: bold;">Optional Activities at Grand World Phu Quoc:</span>',
                    '<span style="color: red;">• Phu Quoc Teddy Bear Museum Admission: $8 USD/pax</span>',
                    '<span style="color: red;">• Venice River Boat Gondola: $8 USD/pax</span>',
                    '<span style="color: red;">• The Quintessence of Vietnam Show Ticket: $12 USD/pax</span>'
                ],
                accommodation: 'Phu Quoc Hotel'
            },
            {
                day: 3,
                title: 'Grand World Free Time - Shopping - Departure',
                meals: 'Breakfast',
                activities: [
                    'Breakfast at hotel',
                    'Free time at Grand World Phuquoc',
                    'Shopping for souvenirs',
                    'End of tour - Transfer to airport or hotel'
                ],
                accommodation: 'End of tour'
            }
        ],
        accommodation: {
            options: [
                {
                    category: '3* Hotel',
                    hotels: ['Standard 3-star accommodations']
                },
                {
                    category: '4* Hotel',
                    hotels: ['VinHolidays Fiesta Phuquoc 4* or similar', 'Wyndham Garden Hotel Phuquoc 4*']
                },
                {
                    category: '5* Hotel',
                    hotels: ['Luxury 5-star accommodations available']
                }
            ]
        },
        meals: {
            total: 'Lunch = 2, Dinner = 2',
            specialties: [
                'Delicious fresh seafood meal in Phu Quoc',
                'Traditional Vietnamese cuisine experience'
            ]
        },
        included: [
            'Accommodation twin share room',
            'Meals: (B) Breakfast, (L) Lunch, (D) Dinner as specified',
            'Transportation with air-conditioning',
            'English/Chinese/Thai speaking guide',
            'Entrance fees to attractions',
            'Drinking water during tours'
        ],
        excluded: [
            'Visa stamping fee in Vietnam',
            'Personal travel insurance',
            'Any flights to and from Vietnam',
            'All personal expenses',
            'All gratuities and tips to driver and tour guide',
            'Early check in & late check out at hotels',
            'Bank fees',
            'Surcharge on Christmas or New Year time',
            'Tips: Guide 2 USD/day, Driver 1 USD/day (Group <10 pax: Guide 4 USD/day, Driver 2 USD/day)'
        ],
        termsAndConditions: [
            'Rates not applicable during peak periods: New Year (28 Dec - 2 Jan), Vietnamese New Year (25 Jan - 2 Feb), holidays (5-7 Apr, 30 Apr - 2 May, 1-3 Sept, 23-25 Dec)',
            'No refund for tickets if tour does not visit attractions except in cases of storms or floods',
            'In cases of force majeure that cannot be resolved, company has right not to refund',
            'Cancellations 7 days before departure (or when company has deposited for hotel, car, etc.) will not be refunded',
            'Company not responsible for expenses due to force majeure events such as flight cancellations/delays, natural disasters, riots, political events, and lost items beyond company control',
            'Travel program may be changed as appropriate without prior notice, considering customer interests as priority'
        ],
        bestTime: 'Year-round tropical destination. Dry season (November-March) offers the best weather conditions.',
        difficulty: 'Easy - Suitable for all ages and fitness levels',
        tags: ['south-vietnam', 'phu-quoc', 'island-tour', 'vinwonders', 'hon-thom', 'premium-attractions', 'aquarium', 'theme-park'],
        featured: true,
        status: 'active',
        lastUpdated: '2025-11-22',
        seoTitle: 'VNS303B Phu Quoc Island Premium Tour with VinWonders | VN To You',
        seoDescription: 'Experience Phu Quoc\'s premier attractions with VinWonders theme park and Hon Thom Islet. Includes Aquarium Phuquoc, Once Show, and premium entertainment in this 3D2N island adventure.',
        seoKeywords: 'VinWonders Phuquoc, Hon Thom Islet, Sun World Nature Park, Aquarium Phuquoc, Once Show, Premium Phu Quoc tour, VinWonders theme park, Phu Quoc attractions'
    },
    {
        id: 'vns401',
        title: 'VNS401 HCM City - Muine - Dalat 4D3N',
        region: 'south',
        category: 'multi-city',
        excerpt: 'Comprehensive 4-day Southern Vietnam tour exploring Ho Chi Minh City, Muine\'s stunning sand dunes, and the charming hill station of Dalat with its cool climate and unique attractions.',
        description: 'VNS401 HCM City - Muine - Dalat 4D3N offers a perfect combination of Vietnam\'s bustling metropolis, coastal desert landscapes, and cool mountain retreat. Experience the White and Red Sand Dunes of Muine, the romantic city of Dalat with its French colonial architecture, and vibrant Ho Chi Minh City.',
        image: {
            url: 'https://vn-to-you-tour.vercel.app/assets/images/tours/vns401.jpg',
            alt: 'VNS401 HCM City Muine Dalat Tour'
        },
        duration: {
            display: '4 Days / 3 Nights',
            days: 4,
            nights: 3
        },
        price: {
            display: 'Please contact us',
            amount: 0,
            currency: 'USD'
        },
        groupSize: 'Private tour - all group sizes (detailed pricing available)',
        highlights: [
            'Explore Muine\'s iconic White and Red Sand Dunes with Jeep adventure',
            'Experience the magical Fairy Stream in Muine',
            'Discover Dalat\'s cool climate and French colonial charm',
            'Visit the famous Crazy House architectural wonder',
            'Explore unique Clay Tunnels in Dalat',
            'Tour Ho Chi Minh City\'s historical landmarks',
            'Experience Dalat night market and local culture',
            'Visit Linh Phuoc Temple and Fresh Garden',
            'Walk along Nguyen Hue Walking Street in HCM City'
        ],
        itinerary: [
            {
                day: 1,
                title: 'HCM City to Muine - Sand Dunes Adventure',
                meals: 'Lunch, Dinner',
                activities: [
                    'Pick up at airport or hotel in HCM City',
                    'Transfer to Muine (approximately 4.5 hours drive)',
                    'Visit White Sand Dunes (include Jeep adventure)',
                    'Explore Red Sand Dunes for sunset viewing',
                    'Walk through the mystical Fairy Stream',
                    'Check-in hotel and dinner'
                ],
                accommodation: 'Muine Hotel (4* options: Muong Thanh Muine 4* or similar)'
            },
            {
                day: 2,
                title: 'Muine to Dalat - Mountain City Discovery',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Transfer from Muine to Dalat (approximately 3.5 hours)',
                    'Visit Fresh Garden with variety of flowers and vegetables',
                    'Explore Linh Phuoc Temple (Dragon Temple)',
                    'Photo stop at Lam Vien Square',
                    'Romantic walk around Xuan Huong Lake',
                    'Experience Dalat night market',
                    'Dinner and overnight in Dalat'
                ],
                accommodation: 'Dalat Hotel (4* options: TTC Dalat Hotel 4*/King Dalat Hotel 4*/SANDALS CAMELLIA HOTEL/Saigon Dalat Hotel 4* or similar)'
            },
            {
                day: 3,
                title: 'Dalat Sightseeing - Return to HCM City',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Visit the famous Crazy House (Hang Nga Guesthouse)',
                    'Explore unique Clay Tunnels',
                    'Transfer back to Ho Chi Minh City (approximately 6 hours)',
                    'Evening stroll along Nguyen Hue Walking Street',
                    'Dinner and overnight in HCM City'
                ],
                accommodation: 'HCM City Hotel (4* options: Cicilia Saigon Hotel 4*/Athena Saigon Hotel 4*/Ramana Saigon Hotel 4* or similar)'
            },
            {
                day: 4,
                title: 'HCM City Tour - Departure',
                meals: 'Breakfast, Lunch',
                activities: [
                    'Breakfast at hotel',
                    'Ho Chi Minh City tour',
                    'Visit Independence Palace (Reunification Palace)',
                    'Explore Saigon Central Post Office',
                    'Shopping at Ben Thanh Market',
                    'Transfer to airport for departure'
                ],
                accommodation: 'End of tour'
            }
        ],
        accommodation: {
            options: [
                {
                    category: '3* Hotel',
                    hotels: ['Standard 3-star accommodations (Not available for this tour)']
                },
                {
                    category: '4* Hotel HCM City',
                    hotels: ['Cicilia Saigon Hotel 4*', 'Athena Saigon Hotel 4*', 'Ramana Saigon Hotel 4*', 'or similar']
                },
                {
                    category: '4* Hotel Muine',
                    hotels: ['Muong Thanh Muine 4*', 'or similar']
                },
                {
                    category: '4* Hotel Dalat',
                    hotels: ['TTC Dalat Hotel 4*', 'King Dalat Hotel 4*', 'SANDALS CAMELLIA HOTEL', 'Saigon Dalat Hotel 4*', 'or similar']
                },
                {
                    category: '5* Hotel',
                    hotels: ['Luxury 5-star options available upon request']
                }
            ]
        },
        meals: {
            total: 'Lunch = 4, Dinner = 3',
            specialties: [
                'Local Muine seafood specialties',
                'Dalat mountain cuisine and fresh vegetables',
                'Traditional Vietnamese dishes in HCM City'
            ]
        },
        included: [
            'Accommodation twin share room',
            'Meals: (B) Breakfast, (L) Lunch, (D) Dinner as specified',
            'Transportation with air-conditioning',
            'English/Chinese/Thai speaking guide',
            'Entrance fees to all attractions',
            'Drinking water during tours'
        ],
        excluded: [
            'Visa stamping fee in Vietnam',
            'Personal travel insurance',
            'Any flights to and from Vietnam',
            'All personal expenses',
            'All gratuities and tips to driver and tour guide',
            'Early check in & late check out at hotels',
            'Bank fees',
            'Surcharge on Christmas or New Year time',
            'Tips: Guide 2 USD/day, Driver 1 USD/day (Group <10 pax: Guide 4 USD/day, Driver 2 USD/day)'
        ],
        termsAndConditions: [
            'Rates not applicable during peak periods: New Year (28 Dec - 2 Jan), Vietnamese New Year (25 Jan - 2 Feb), holidays (5-7 Apr, 30 Apr - 2 May, 1-3 Sept, 23-25 Dec)',
            'No refund for tickets if tour does not visit attractions except in cases of storms or floods',
            'In cases of force majeure that cannot be resolved, company has right not to refund',
            'Cancellations 7 days before departure (or when company has deposited for hotel, car, etc.) will not be refunded',
            'Company not responsible for expenses due to force majeure events such as flight cancellations/delays, natural disasters, riots, political events, and lost items beyond company control',
            'Travel program may be changed as appropriate without prior notice, considering customer interests as priority'
        ],
        bestTime: 'Year-round destination. Dry season (November-March) is optimal for comfortable weather in all three locations.',
        difficulty: 'Easy to Moderate - Suitable for most fitness levels with some walking involved',
        tags: ['south-vietnam', 'multi-city', 'hcm-city', 'muine', 'dalat', 'sand-dunes', 'mountain-retreat', 'cultural-tour'],
        featured: true,
        status: 'active',
        lastUpdated: '2025-11-22',
        seoTitle: 'VNS401 HCM City Muine Dalat 4D3N Tour | VN To You',
        seoDescription: 'Explore Southern Vietnam\'s diverse landscapes with our 4D3N tour covering bustling HCM City, stunning Muine sand dunes, and charming Dalat mountain retreat with detailed pricing options.',
        seoKeywords: 'HCM City Muine Dalat tour, White Red Sand Dunes Muine, Crazy House Dalat, Southern Vietnam multi-city tour, Vietnam 4 days tour, Dalat mountain tour'
    },
    {
        id: 'vns403',
        title: 'VNS403 Phuquoc 4D3N',
        region: 'south',
        category: 'comprehensive',
        excerpt: 'Complete 4-day Phu Quoc island experience featuring Grand World entertainment complex, VinWonders theme park, Hon Thom Island cable car, and all major attractions with 3 nights accommodation.',
        description: 'VNS403 Phuquoc 4D3N offers the most comprehensive Phu Quoc island experience combining entertainment, adventure, and relaxation. Enjoy 3 nights on the North Island with visits to Grand World, VinWonders, Hon Thom Island, fresh seafood dining, and traditional Vietnamese cuisine.',
        image: {
            url: 'https://vn-to-you-tour.vercel.app/assets/images/tours/vns403.jpg',
            alt: 'VNS403 Phuquoc 4D3N Comprehensive Tour'
        },
        duration: {
            display: '4 Days / 3 Nights',
            days: 4,
            nights: 3
        },
        price: {
            display: 'Please contact us',
            amount: 0,
            currency: 'USD'
        },
        groupSize: 'Private tour for 2-10 people',
        highlights: [
            'A delicious fresh seafood meal in Phu Quoc',
            'One experience of traditional Vietnamese cuisine',
            '3 nights stay on the North Island of Phu Quoc',
            'Grand World Phuquoc - The Sleepless City with nightly light show',
            'Full day at VinWonders Phuquoc theme park',
            'Aquarium Phuquoc and Once Show experience',
            'Hon Thom Island cable car adventure with Aquatopia Water Park',
            'Sunset Town and Kiss Bridge romantic visit',
            'Photo stop at iconic Dinh Cau Temple',
            'Multiple hotel options from 3* to 5*',
            'Comprehensive coverage of all major Phu Quoc attractions'
        ],
        itinerary: [
            {
                day: 1,
                title: 'Airport Pickup - Grand World Phuquoc - Light Show',
                meals: 'Lunch, Dinner',
                activities: [
                    'Pick up at airport (or hotel)',
                    'Transfer to Grand World Phuquoc (The Sleepless City)',
                    'Photo stop at iconic Dinh Cau Temple',
                    'Free time exploration at Grand World Phuquoc',
                    'Grand World Nightly Light Show experience',
                    'Dinner featuring fresh seafood specialties',
                    '<span style="color: red; font-weight: bold;">Optional Activities at Grand World Phu Quoc:</span>',
                    '<span style="color: red;">• Phu Quoc Teddy Bear Museum Admission: $8 USD/pax</span>',
                    '<span style="color: red;">• Venice River Boat Gondola: $8 USD/pax</span>',
                    '<span style="color: red;">• The Quintessence of Vietnam Show Ticket: $12 USD/pax</span>'
                ],
                accommodation: 'Phu Quoc Hotel'
            },
            {
                day: 2,
                title: 'VinWonders Phuquoc - Aquarium - Once Show',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Full day visit to VinWonders Phuquoc theme park',
                    'Explore Aquarium Phuquoc with marine life exhibits',
                    'Experience spectacular Once Show Phuquoc',
                    'Dinner with traditional Vietnamese cuisine'
                ],
                accommodation: 'Phu Quoc Hotel'
            },
            {
                day: 3,
                title: 'Hon Thom Island - Aquatopia Water Park - Sunset Town',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Full day visit to Hon Thom Island',
                    'Aquatopia Water Park experience (include cable car, buffet)',
                    'Visit romantic Sunset Town',
                    'Photo session at famous Kiss Bridge',
                    'Dinner on Phu Quoc island',
                    '<span style="color: red; font-weight: bold;">Optional Shows at Sunset Town:</span>',
                    '<span style="color: red;">• Symphony of the Sea Show (7:45 PM – 8:10 PM): $12 USD/pax</span>',
                    '<span style="color: red;">• Kiss of the Sea main show with fireworks (9:00 PM – 9:30 PM): $45 USD for two people or $35 USD for one person (Note: Not performed on Tuesdays)</span>'
                ],
                accommodation: 'Phu Quoc Hotel'
            },
            {
                day: 4,
                title: 'Shopping - Departure',
                meals: 'Breakfast',
                activities: [
                    'Breakfast at hotel',
                    'Free time for shopping and souvenir hunting',
                    'Last-minute exploration of local markets',
                    'End of tour - Transfer to airport or hotel'
                ],
                accommodation: 'End of tour'
            }
        ],
        accommodation: {
            options: [
                {
                    category: '3* Hotel',
                    hotels: ['Standard 3-star accommodations available']
                },
                {
                    category: '4* Hotel',
                    hotels: ['VinHolidays Fiesta Phuquoc 4* or similar', 'Wyndham Garden Hotel Phuquoc 4*']
                },
                {
                    category: '5* Hotel',
                    hotels: ['Luxury 5-star accommodations available']
                }
            ]
        },
        meals: {
            total: 'Lunch = 3, Dinner = 3',
            specialties: [
                'Delicious fresh seafood meal in Phu Quoc',
                'Traditional Vietnamese cuisine experience',
                'Island specialties and local delicacies'
            ]
        },
        included: [
            'Accommodation twin share room',
            'Meals: (B) Breakfast, (L) Lunch, (D) Dinner as specified',
            'Transportation with air-conditioning',
            'English/Chinese/Thai speaking guide',
            'Entrance fees to all attractions',
            'Drinking water during tours'
        ],
        excluded: [
            'Visa stamping fee in Vietnam',
            'Personal travel insurance',
            'Any flights to and from Vietnam',
            'All personal expenses',
            'All gratuities and tips to driver and tour guide',
            'Early check in & late check out at hotels',
            'Bank fees',
            'Surcharge on Christmas or New Year time',
            'Tips: Guide 2 USD/day, Driver 1 USD/day (Group <10 pax: Guide 4 USD/day, Driver 2 USD/day)'
        ],
        termsAndConditions: [
            'Rates not applicable during peak periods: New Year (28 Dec - 2 Jan), Vietnamese New Year (25 Jan - 2 Feb), holidays (5-7 Apr, 30 Apr - 2 May, 1-3 Sept, 23-25 Dec)',
            'No refund for tickets if tour does not visit attractions except in cases of storms or floods',
            'In cases of force majeure that cannot be resolved, company has right not to refund',
            'Cancellations 7 days before departure (or when company has deposited for hotel, car, etc.) will not be refunded',
            'Company not responsible for expenses due to force majeure events such as flight cancellations/delays, natural disasters, riots, political events, and lost items beyond company control',
            'Travel program may be changed as appropriate without prior notice, considering customer interests as priority'
        ],
        bestTime: 'Year-round tropical destination. Dry season (November-March) offers the best weather conditions.',
        difficulty: 'Easy - Suitable for all ages and fitness levels',
        tags: ['south-vietnam', 'phu-quoc', 'island-tour', 'comprehensive', 'grand-world', 'vinwonders', 'hon-thom', 'aquatopia', 'sunset-town'],
        featured: true,
        status: 'active',
        lastUpdated: '2025-11-22',
        seoTitle: 'VNS403 Phuquoc 4D3N Comprehensive Island Tour | VN To You',
        seoDescription: 'Ultimate 4D3N Phu Quoc island experience with Grand World, VinWonders, Hon Thom Island, Aquatopia Water Park, and all major attractions. Complete island adventure with fresh seafood and entertainment.',
        seoKeywords: 'Phuquoc 4D3N tour, VinWonders Phuquoc, Grand World Phuquoc, Hon Thom Island, Aquatopia Water Park, Phu Quoc comprehensive tour, Vietnam island vacation, Sunset Town Kiss Bridge'
    },
    {
        id: 'vngt001',
        title: 'Vietnam Grand Tour - Complete Vietnam Discovery',
        region: 'multi',
        category: 'comprehensive',
        excerpt: 'The ultimate Vietnam experience covering all three regions - from the bustling streets of Ho Chi Minh City to the majestic Halong Bay, discovering the full beauty and culture of Vietnam in one comprehensive journey.',
        description: 'Vietnam Grand Tour offers the most complete Vietnam experience, combining the best of all three regions. Start from dynamic Southern Vietnam with Ho Chi Minh City and Mekong Delta, journey through the historic Central region with Hoi An and Hue, and conclude with the spectacular Northern Vietnam featuring Hanoi and Halong Bay. This comprehensive tour showcases Vietnam\'s diverse landscapes, rich history, and vibrant culture.',
        image: {
            url: 'https://vn-to-you-tour.vercel.app/assets/images/tours/vietnam-grand-tour.jpg',
            alt: 'Vietnam Grand Tour - Complete Vietnam Discovery'
        },
        duration: {
            display: '12 Days / 11 Nights',
            days: 12,
            nights: 11
        },
        price: {
            display: 'Please contact for detailed pricing',
            amount: 0,
            currency: 'USD'
        },
        groupSize: 'Private tour for 2-12 people',
        highlights: [
            'Complete Vietnam experience covering all three regions',
            'Ho Chi Minh City cultural and historical exploration',
            'Authentic Mekong Delta floating market experience',
            'Hoi An Ancient Town UNESCO World Heritage site',
            'Imperial Hue with Forbidden Purple City',
            'Hanoi Old Quarter and cultural immersion',
            'Spectacular Halong Bay overnight cruise experience',
            'Traditional water puppet show in Hanoi',
            'Vietnamese cooking classes and street food tours',
            'Professional English-speaking guide throughout',
            'Carefully selected 4-5 star accommodations',
            'All domestic flights included'
        ],
        itinerary: [
            {
                day: 1,
                title: 'Arrival Ho Chi Minh City',
                meals: 'Dinner',
                activities: [
                    'Airport pickup and transfer to hotel',
                    'Check-in and rest',
                    'Welcome dinner at local restaurant',
                    'Evening briefing about the tour'
                ],
                accommodation: 'Ho Chi Minh City Hotel (4-5*)'
            },
            {
                day: 2,
                title: 'Ho Chi Minh City Full Day Tour',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Visit Reunification Palace',
                    'War Remnants Museum exploration',
                    'Notre Dame Cathedral and Central Post Office',
                    'Ben Thanh Market shopping experience',
                    'Cu Chi Tunnels half-day tour',
                    'Traditional Vietnamese dinner with cultural show'
                ],
                accommodation: 'Ho Chi Minh City Hotel (4-5*)'
            },
            {
                day: 3,
                title: 'Mekong Delta Adventure',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Early departure to Mekong Delta',
                    'Boat cruise through narrow canals',
                    'Visit floating market',
                    'Traditional handicraft workshops',
                    'Local family lunch in orchard garden',
                    'Return to Ho Chi Minh City'
                ],
                accommodation: 'Ho Chi Minh City Hotel (4-5*)'
            },
            {
                day: 4,
                title: 'Flight to Da Nang - Hoi An Ancient Town',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Morning flight to Da Nang',
                    'Transfer to Hoi An Ancient Town',
                    'Check-in at resort',
                    'Afternoon exploring Hoi An Old Quarter',
                    'Japanese Covered Bridge visit',
                    'Traditional lantern-lit dinner'
                ],
                accommodation: 'Hoi An Resort (4-5*)'
            },
            {
                day: 5,
                title: 'Hoi An Countryside & Cooking Class',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Bicycle tour through rice paddies',
                    'Visit local farming villages',
                    'Traditional basket boat experience',
                    'Vietnamese cooking class with market tour',
                    'Enjoy your self-prepared lunch',
                    'Afternoon at leisure or beach time'
                ],
                accommodation: 'Hoi An Resort (4-5*)'
            },
            {
                day: 6,
                title: 'Hoi An to Hue - Imperial City',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Scenic drive via Hai Van Pass',
                    'Photo stops at Lang Co Beach',
                    'Check-in Hue hotel',
                    'Afternoon visit to Imperial Citadel',
                    'Forbidden Purple City exploration',
                    'Royal tomb visit (Khai Dinh or Tu Duc)'
                ],
                accommodation: 'Hue Hotel (4-5*)'
            },
            {
                day: 7,
                title: 'Hue Cultural Immersion - Flight to Hanoi',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Morning boat trip on Perfume River',
                    'Thien Mu Pagoda visit',
                    'Local market exploration',
                    'Flight to Hanoi',
                    'Transfer to hotel and check-in',
                    'Welcome dinner in Hanoi'
                ],
                accommodation: 'Hanoi Hotel (4-5*)'
            },
            {
                day: 8,
                title: 'Hanoi City Discovery',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Ho Chi Minh Mausoleum (exterior)',
                    'One Pillar Pagoda visit',
                    'Temple of Literature exploration',
                    'Hanoi Old Quarter walking tour',
                    'Traditional Vietnamese lunch',
                    'Water Puppet Show in the evening',
                    'Street food tour by night'
                ],
                accommodation: 'Hanoi Hotel (4-5*)'
            },
            {
                day: 9,
                title: 'Hanoi to Halong Bay Cruise',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Transfer to Halong Bay (3.5 hours)',
                    'Board luxury cruise ship',
                    'Cruise through limestone karsts',
                    'Cave exploration (Sung Sot or Thien Cung)',
                    'Kayaking or bamboo boat experience',
                    'Sunset cocktail on deck',
                    'Fresh seafood dinner on board'
                ],
                accommodation: 'Halong Bay Cruise (4-5* ship)'
            },
            {
                day: 10,
                title: 'Halong Bay Exploration - Return Hanoi',
                meals: 'Breakfast, Lunch',
                activities: [
                    'Early morning Tai Chi session on deck',
                    'Visit floating fishing village',
                    'Pearl farming demonstration',
                    'Final cruise through the bay',
                    'Disembark and return to Hanoi',
                    'Free time for shopping at Dong Xuan Market'
                ],
                accommodation: 'Hanoi Hotel (4-5*)'
            },
            {
                day: 11,
                title: 'Hanoi Cultural Experience',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Visit Museum of Ethnology',
                    'Ngoc Son Temple on Hoan Kiem Lake',
                    'Cyclo tour through Old Quarter',
                    'Traditional handicraft shopping',
                    'Farewell dinner with traditional music',
                    'Last night in Vietnam preparations'
                ],
                accommodation: 'Hanoi Hotel (4-5*)'
            },
            {
                day: 12,
                title: 'Departure',
                meals: 'Breakfast',
                activities: [
                    'Breakfast at hotel',
                    'Free time until airport transfer',
                    'Shopping for last-minute souvenirs',
                    'Transfer to Noi Bai Airport',
                    'End of Vietnam Grand Tour'
                ],
                accommodation: 'End of tour'
            }
        ],
        accommodation: {
            hoChiMinhCity: {
                options: [
                    'Liberty Central Saigon Citypoint (4*)',
                    'Park Hyatt Saigon (5*)',
                    'Hotel des Arts Saigon (5*)'
                ]
            },
            hoiAn: {
                options: [
                    'Hoi An Eco Lodge & Spa (4*)',
                    'Anantara Hoi An Resort (5*)',
                    'Four Seasons Resort The Nam Hai (5*)'
                ]
            },
            hue: {
                options: [
                    'Moonlight Hotel Hue (4*)',
                    'Pilgrimage Village Boutique Resort (5*)',
                    'Azerai La Residence Hue (5*)'
                ]
            },
            hanoi: {
                options: [
                    'Hanoi Pearl Hotel (4*)',
                    'Sofitel Legend Metropole Hanoi (5*)',
                    'JW Marriott Hotel Hanoi (5*)'
                ]
            },
            halongBay: {
                options: [
                    'Paradise Luxury Cruise (4*)',
                    'Heritage Binh Chuan Cruise (5*)',
                    'Stellar of the Seas Cruise (5*)'
                ]
            }
        },
        meals: {
            total: 'B=11, L=10, D=10',
            specialties: [
                'Welcome dinner with cultural show',
                'Mekong Delta local family lunch',
                'Vietnamese cooking class meal',
                'Royal cuisine in Hue',
                'Halong Bay fresh seafood',
                'Hanoi street food experience',
                'Farewell dinner with traditional music'
            ]
        },
        included: [
            'Accommodation in 4-5* hotels and luxury cruise',
            'All meals as specified in itinerary',
            'Professional English-speaking guide throughout',
            'All transportation including domestic flights',
            'Entrance fees to all attractions',
            'Boat trips and cruise experiences',
            'Cooking class and cultural activities',
            'Airport transfers',
            'Drinking water during tours'
        ],
        excluded: [
            'International flights to/from Vietnam',
            'Vietnam visa fees',
            'Personal travel insurance',
            'Personal expenses and shopping',
            'Tips for guides and drivers',
            'Additional meals not specified',
            'Alcoholic beverages',
            'Spa treatments and optional activities',
            'Single room supplement'
        ],
        termsAndConditions: [
            'Minimum 2 passengers required for private tour',
            'Peak season surcharges apply during Tet holidays and Christmas/New Year',
            'Domestic flight schedules subject to change',
            'Weather conditions may affect Halong Bay cruise activities',
            'Hotels subject to availability, similar standard alternatives may be used',
            'Cancellation policy: 30% charge for 7-14 days notice, 50% for 3-6 days, 100% for less than 3 days',
            'Force majeure events may require itinerary adjustments'
        ],
        bestTime: 'Best time: March-May and September-November for ideal weather across all regions. Avoid rainy season (June-August) in the north.',
        difficulty: 'Easy to Moderate - Suitable for most fitness levels with moderate walking involved',
        tags: ['grand-tour', 'all-regions', 'comprehensive', 'cultural', 'heritage', 'halong-bay', 'mekong-delta', 'hoi-an', 'hue', 'hanoi', 'ho-chi-minh-city'],
        featured: true,
        status: 'active',
        lastUpdated: '2025-12-11',
        seoTitle: 'Vietnam Grand Tour 12D11N - Complete Vietnam Discovery | VN To You',
        seoDescription: 'Ultimate 12-day Vietnam Grand Tour covering all three regions from Ho Chi Minh City to Halong Bay. Experience Mekong Delta, Hoi An, Hue Imperial City, Hanoi culture, and luxury Halong cruise in one complete journey.',
        seoKeywords: 'Vietnam Grand Tour, complete Vietnam tour, 12 days Vietnam, Halong Bay cruise, Mekong Delta tour, Hoi An Hue Hanoi, Vietnam cultural tour, comprehensive Vietnam travel, luxury Vietnam tour'
    },
    {
        id: 'vng501',
        title: 'VNG501 Hanoi – Sapa – Da Nang: Vietnam Grand Tour',
        region: 'multi',
        category: 'grand-tour',
        excerpt: 'Featuring the two most popular and exciting travel destinations in Vietnam today - Experience Sapa\'s mountain beauty and Da Nang\'s coastal charm in one comprehensive 5-day tour.',
        description: 'VNG501 Hanoi – Sapa – Da Nang combines the stunning mountain landscapes of Sapa with the beautiful coastal attractions of Da Nang. This comprehensive tour features signature dining experiences including Salmon Hotpot in Sapa, fresh seafood in Da Nang, and visits to iconic attractions like Fansipan, Ba Na Hills, and Hoi An Ancient Town.',
        image: {
            url: 'https://vn-to-you-tour.vercel.app/assets/images/tours/vng501.jpg',
            alt: 'VNG501 Hanoi Sapa Da Nang Vietnam Grand Tour'
        },
        duration: {
            display: '5 Days / 4 Nights',
            days: 5,
            nights: 4
        },
        price: {
            display: 'Please contact us for pricing',
            amount: 0,
            currency: 'USD'
        },
        groupSize: 'Private tour for 2-10 people',
        highlights: [
            'Signature Sapa Dinner: Enjoy Salmon Hotpot and Sashimi paired with Dalat Wine, Vietnam\'s famous vintage, amidst Sapa\'s cool, misty air',
            'Fansipan Buffet offers a wide array of dishes, highlighting the diverse cuisine of Sapa',
            'A seafood hotpot featuring fresh Da Nang seafood and half a lobster for each guest',
            'Buffet Ba Na Hills with Golden Hand Bridge visit',
            'Visit Cat Cat Village and explore Sapa\'s ethnic culture',
            'Explore Hoi An Ancient Town, Japanese Covered Bridge, and traditional houses',
            'Shopping at Sapa night market',
            'Multiple hotel options from 3* to 5* in Sapa, Hanoi, and Da Nang'
        ],
        itinerary: [
            {
                day: 1,
                title: 'Arrival - Transfer to Sapa - Cat Cat Village',
                meals: 'Lunch, Dinner',
                activities: [
                    'Pick up at airport (or hotel)',
                    'Transfer to Sa Pa',
                    'Visit Cat Cat Village',
                    'Dinner (Salmon hotpot + Dalat wine)',
                    'Shopping at night market Sapa'
                ],
                accommodation: 'Sapa Hotel (3*/4*/5* options available)'
            },
            {
                day: 2,
                title: 'Fansipan Peak Adventure',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Visit Fansipan (include Muong Hoa Train, Buffet Fansipan, Cable Car ticket)',
                    'Visit Moana Cafe (cafe fee not included)',
                    'Back to Hanoi'
                ],
                accommodation: 'Hanoi Hotel (3*/4*/5* options available)',
                notes: 'Optional Funicular Train Fansipan: $7 USD/pax up, $5 USD/pax down (Ideal for elderly guests who wish to skip the 600-step climb to the peak)'
            },
            {
                day: 3,
                title: 'Hanoi to Da Nang - City Tour',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Back to Hanoi',
                    'Tran Quoc Pagoda',
                    'Hanoi Old Quarter',
                    'Sword Lake Hanoi',
                    'Shopping',
                    'Railway cafe Hanoi (cafe fee not included)',
                    'Flight to Da Nang (airfare not included)'
                ],
                accommodation: 'Da Nang Hotel (3*/4*/5* options available)'
            },
            {
                day: 4,
                title: 'Full Day Ba Na Hills Adventure',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Full day visit to Ba Na Hills',
                    'Transfer to Ba Na Hills via cable car',
                    'Visit French Village',
                    'Moon Castle exploration',
                    'Fantasy Park experience',
                    'Visit Golden Hand Bridge',
                    'Le Jardin D\'Amour Flower Garden',
                    'Buffet lunch at Ba Na Hills',
                    'Return to Da Nang'
                ],
                accommodation: 'Da Nang Hotel (3*/4*/5* options available)',
                notes: 'Optional activities: Wax Museum ($4 USD/pax), Alpine Coaster ($4 USD/pax), Wine Cellar ($4 USD/pax)'
            },
            {
                day: 5,
                title: 'Hoi An Ancient Town - Departure',
                meals: 'Breakfast, Lunch',
                activities: [
                    'Breakfast at hotel',
                    'Transfer to Hoian',
                    'Visit Hoian Ancient Town',
                    'Japanese Covered Bridge',
                    'Tan Ky Old House',
                    'Assembly Hall (Hoi Quan Phuoc Kien)',
                    'Back to Danang',
                    'End of tour - Transfer to airport'
                ],
                accommodation: 'End of tour'
            }
        ],
        accommodations: {
            sapa: {
                '3star': [
                    'Azure Sapa 3*',
                    'Sapa Relax Hotel 3*',
                    'Or similar'
                ],
                '4star': [
                    'Sapa Highland Resort & Spa 4*',
                    'Charm Sapa Hotel 4*',
                    'Hotel De Sapa 4*',
                    'Or similar'
                ],
                '5star': [
                    'Available upon request'
                ]
            },
            hanoi: {
                '3star': [
                    'CWD Hanoi Hotel 3*',
                    'Quang Ba Trade Union Hotel 3*',
                    'Or similar'
                ],
                '4star': [
                    'Thang Long Opera Hotel 4*',
                    'Muong Thanh Central Hotel 4*',
                    'Super Candle Hanoi Hotel 4*',
                    'Lacasa Hanoi Hotel 4*',
                    'Or similar'
                ],
                '5star': [
                    'Available upon request'
                ]
            },
            danang: {
                '3star': [
                    'Sepon Blue Hotel 3*',
                    'Pandora Danang Hotel',
                    'Or similar'
                ],
                '4star': [
                    'Luxtery Da Nang 4*',
                    'Nhu Minh Danang 4*',
                    'Or similar'
                ],
                '5star': [
                    'Golden Bay Danang Hotel 5*',
                    'Muong Thanh Luxury Danang 5*',
                    'Or similar'
                ]
            }
        },
        meals: {
            total: 'Lunch = 5, Dinner = 4',
            specialties: [
                'Signature Sapa Dinner: Salmon Hotpot and Sashimi with Dalat Wine',
                'Fansipan Buffet with diverse Sapa cuisine',
                'Seafood hotpot with fresh Da Nang seafood and half lobster per guest',
                'Ba Na Hills Buffet'
            ]
        },
        included: [
            'Accommodation twin share room (1 night Sapa, 1 night Hanoi, 2 nights Da Nang)',
            'Meals: (B) Breakfast, (L) Lunch, (D) Dinner as specified',
            'Transportation with air-conditioning',
            'English / Chinese / Thai speaking guide',
            'Entrance fees to all attractions',
            'Drinking water during tours'
        ],
        excluded: [
            'Visa stamping fee in Vietnam',
            'Personal travel insurance',
            'Any flights to and from Vietnam (including Hanoi to Da Nang flight)',
            'All personal expenses',
            'All gratuities and tips to driver and tour guide',
            'Early check in & late check out at hotels',
            'Bank fees',
            'Surcharge on Christmas or New Year time',
            'Tips: Guide $2 USD/day, Driver $1 USD/day (Group <10 pax: Guide $4 USD/day, Driver $2 USD/day)'
        ],
        termsAndConditions: [
            'Rates are not applicable during: New Year 28 Dec - 2 Jan 2026 | Vietnamese New Year 15 - 20 Feb 2026 | 27 Apr 2026 | 30 Apr - 2 May 2026 | 1 - 3 Sept 2026 | 23 - 25 DEC 2026',
            'Tickets no refund in case of the tour does not visit various tourist attractions except in the event of a storm or flood',
            'When there is a force majeure that cannot be solved, the company has the right not to refund in all cases',
            'Cancellations 07 days before departure (or in the case of the company Deposit for hotel, car, etc.) will not return the money',
            'The company has the right not to be responsible for any expenses in the event of force majeure such as cancellation or delay of airlines / natural disasters / riots / political events and lost items at places where beyond the control of the company',
            'The travel program may be changed as appropriate without prior notice. The company will take into account the interests of customers as a priority'
        ],
        bestTime: 'Best time: March-May and September-November for ideal weather in both Sapa and Da Nang. Avoid peak summer (June-August) for cooler experience.',
        difficulty: 'Moderate - Suitable for most fitness levels with some walking and stair climbing at attractions',
        tags: ['vietnam-grand-tour', 'multi-region', 'sapa', 'hanoi', 'danang', 'hoian', 'fansipan', 'ba-na-hills', 'comprehensive', 'cultural-heritage'],
        featured: true,
        status: 'active',
        lastUpdated: '2025-12-11',
        seoTitle: 'VNG501 Hanoi Sapa Da Nang 5D4N Grand Tour | VN To You',
        seoDescription: 'Experience Vietnam\'s two most popular destinations in one tour. 5-day journey from Sapa\'s mountains to Da Nang\'s coast, featuring Fansipan, Salmon Hotpot, Ba Na Hills, Golden Hand Bridge, and Hoi An Ancient Town.',
        seoKeywords: 'Vietnam Grand Tour, Hanoi Sapa Danang tour, Fansipan peak, Ba Na Hills, Golden Hand Bridge, Hoi An ancient town, salmon hotpot Sapa, comprehensive Vietnam tour, multi-region Vietnam'
    },
    {
        id: 'vng502',
        title: 'VNG502 Hanoi – Sapa – Halong Bay Cruise',
        region: 'multi',
        category: 'grand-tour',
        excerpt: 'Experience the perfect combination of Sapa\'s mountain beauty and Halong Bay\'s coastal magnificence in this 5-day grand tour featuring overnight cruise, Fansipan peak, and cultural heritage sites.',
        description: 'VNG502 Hanoi – Sapa – Halong Bay Cruise offers an unforgettable journey through Northern Vietnam\'s most iconic destinations. Experience Sapa\'s cool mountain climate with signature Salmon Hotpot dinner, conquer Fansipan peak, explore Hanoi\'s cultural sites, and cruise through the magnificent Halong Bay on a luxury 5-star ship. This comprehensive tour includes 2 nights in Sapa, 2 nights in Hanoi, and 1 night aboard a premium Halong Bay cruise.',
        image: {
            url: 'https://vn-to-you-tour.vercel.app/assets/images/tours/vng502.jpg',
            alt: 'VNG502 Hanoi Sapa Halong Bay Cruise Grand Tour'
        },
        duration: {
            display: '5 Days / 4 Nights',
            days: 5,
            nights: 4
        },
        price: {
            display: 'Please contact us for pricing',
            amount: 0,
            currency: 'USD'
        },
        groupSize: 'Private tour for 2-10 people',
        highlights: [
            'Enjoy a fine-dining dinner on board luxury Halong Bay cruise',
            'Signature Sapa Dinner: Salmon Hotpot and Sashimi paired with Dalat Wine in Sapa\'s cool, misty air',
            'Fansipan Buffet offers a wide array of dishes, highlighting the diverse cuisine of Sapa',
            'Visit Cat Cat Village and explore Sapa\'s ethnic culture',
            'Conquer Fansipan peak with cable car and Muong Hoa Train',
            'Explore Halong Bay caves: Bo Nau (Pelican Cave) and Sung Sot (Surprising Cave)',
            'Swimming and relaxing at Titov Island',
            'Hanoi cultural tour including Tran Quoc Pagoda, West Lake, and Old Quarter',
            'Cooking class and Tai Chi session on cruise',
            'Shopping at Sapa night market',
            'Luxury 5-star Halong Bay cruise experience',
            '2 nights Sapa, 2 nights Hanoi, 1 night Halong Bay cruise'
        ],
        itinerary: [
            {
                day: 1,
                title: 'Arrival - Transfer to Sapa - Cat Cat Village',
                meals: 'Lunch, Dinner',
                activities: [
                    'Pick up at airport (or hotel)',
                    'Transfer to Sapa',
                    'Visit Cat Cat Village',
                    'Shopping at night market Sapa',
                    'Overnight in Sapa'
                ],
                accommodation: 'Sapa Hotel (4* options available)'
            },
            {
                day: 2,
                title: 'Fansipan Peak Adventure',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Visit Fansipan (include Muong Hoa Train, Buffet Fansipan, Cable Car ticket)',
                    'Visit Moana Cafe (cafe fee not included)',
                    '<span style="color: red; font-weight: bold;">Optional Funicular Train Fansipan:</span>',
                    '<span style="color: red;">• $7 USD/pax up, $5 USD/pax down (Ideal for elderly guests who wish to skip the 600-step climb to the peak)</span>',
                    'Signature dinner: Salmon hotpot + Dalat wine',
                    'Overnight in Sapa'
                ],
                accommodation: 'Sapa Hotel (4* options available)'
            },
            {
                day: 3,
                title: 'Sapa to Hanoi - City Tour',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Transfer back to Hanoi',
                    'City tour Hanoi',
                    'Visit Tran Quoc Pagoda',
                    'West Lake sightseeing',
                    'Hanoi Old Quarter exploration',
                    'Cafe Railway (cafe fee not included)',
                    'Overnight in Hanoi'
                ],
                accommodation: 'Hanoi Hotel (4* options available)'
            },
            {
                day: 4,
                title: 'Hanoi to Halong Bay - Luxury Cruise Experience',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Transfer to Halong Bay',
                    'Check-in to luxury cruise',
                    'Cruise through magnificent limestone karsts',
                    'Visit "Bo Nau" cave (Pelican Cave)',
                    'Explore "Sung Sot" Cave (Surprising Cave)',
                    'Enjoy swimming and relaxing on the beach',
                    'Climb to the top of Titov Island for panoramic views',
                    'Cooking class on board',
                    'Fine-dining dinner on cruise',
                    'Overnight on luxury cruise'
                ],
                accommodation: 'Halong Bay 5* Cruise (Amanda/Rosy/Rita/Aqua of The Seas)',
                notes: 'Our overnight cruise programs in Ha Long Bay operate on two routes: Ha Long Bay and Lan Ha Bay. The itinerary may vary depending on each cruise provider.'
            },
            {
                day: 5,
                title: 'Halong Bay Morning - Departure',
                meals: 'Breakfast, Lunch',
                activities: [
                    'Early morning Tai Chi class on deck',
                    'Breakfast on cruise',
                    'Visit "Luon" cave by kayak or bamboo boat',
                    'Check-out from cruise',
                    'Transfer to airport',
                    'End of tour'
                ],
                accommodation: 'End of tour'
            }
        ],
        accommodations: {
            sapa: {
                '4star': [
                    'Sapa Highland Resort & Spa 4*',
                    'Charm Sapa Hotel 4*',
                    'Hotel De Sapa 4*',
                    'Or similar'
                ]
            },
            hanoi: {
                '4star': [
                    'Thang Long Opera Hotel 4*',
                    'Muong Thanh Central Hotel 4*',
                    'Super Candle Hanoi Hotel 4*',
                    'Lacasa Hanoi Hotel 4*',
                    'Or similar'
                ]
            },
            halongBay: {
                '5star': [
                    'Amanda Cruise 5*',
                    'Rosy Cruises 5*',
                    'Rita Cruises 5*',
                    'Aqua of The Seas 5*',
                    'Or similar'
                ]
            }
        },
        meals: {
            total: 'Lunch = 5, Dinner = 4',
            specialties: [
                'Signature Sapa Dinner: Salmon Hotpot and Sashimi with Dalat Wine',
                'Fansipan Buffet with diverse Sapa cuisine',
                'Fine-dining dinner on board luxury cruise',
                'Fresh seafood on Halong Bay cruise'
            ]
        },
        included: [
            'Accommodation twin share room (2 nights Sapa, 2 nights Hanoi, 1 night Halong Bay cruise)',
            'Meals: (B) Breakfast, (L) Lunch, (D) Dinner as specified',
            'Transportation with air-conditioning',
            'English / Chinese / Thai speaking guide',
            'Entrance fees to all attractions',
            'Drinking water during tours'
        ],
        excluded: [
            'Visa stamping fee in Vietnam',
            'Personal travel insurance',
            'Any flights to and from Vietnam',
            'All personal expenses',
            'All gratuities and tips to driver and tour guide',
            'Early check in & late check out at hotels',
            'Bank fees',
            'Surcharge on Christmas or New Year time',
            'Tips: Guide $2 USD/day, Driver $1 USD/day (Group <10 pax: Guide $4 USD/day, Driver $2 USD/day)'
        ],
        termsAndConditions: [
            'Rates are not applicable during: New Year 28 Dec - 2 Jan 2026 | Vietnamese New Year 15 - 20 Feb 2026 | 27 Apr 2026 | 30 Apr - 2 May 2026 | 1 - 3 Sept 2026 | 23 - 25 DEC 2026',
            'Tickets no refund in case of the tour does not visit various tourist attractions except in the event of a storm or flood',
            'When there is a force majeure that cannot be solved, the company has the right not to refund in all cases',
            'Cancellations 07 days before departure (or in the case of the company Deposit for hotel, car, etc.) will not return the money',
            'The company has the right not to be responsible for any expenses in the event of force majeure such as cancellation or delay of airlines / natural disasters / riots / political events and lost items at places where beyond the control of the company',
            'The travel program may be changed as appropriate without prior notice. The company will take into account the interests of customers as a priority'
        ],
        bestTime: 'Best time: March-May and September-November for ideal weather in both Sapa and Halong Bay. Avoid rainy season (June-August) in the north.',
        difficulty: 'Moderate - Suitable for most fitness levels with some walking and stair climbing at attractions',
        tags: ['vietnam-grand-tour', 'multi-region', 'sapa', 'hanoi', 'halong-bay', 'fansipan', 'cruise', 'luxury-cruise', 'comprehensive', 'cultural-heritage', 'northern-vietnam'],
        featured: true,
        status: 'active',
        lastUpdated: '2025-12-11',
        seoTitle: 'VNG502 Hanoi Sapa Halong Bay Cruise 5D4N Grand Tour | VN To You',
        seoDescription: 'Experience Northern Vietnam\'s best destinations in 5 days. Explore Sapa mountains with Fansipan peak, enjoy signature Salmon Hotpot, discover Hanoi culture, and cruise through magnificent Halong Bay on a luxury 5-star ship.',
        seoKeywords: 'Vietnam Grand Tour, Hanoi Sapa Halong Bay tour, Fansipan peak, luxury Halong Bay cruise, Sapa salmon hotpot, Northern Vietnam tour, comprehensive Vietnam travel, 5-star cruise Halong'
    },
    {
        id: 'vng701',
        title: 'VNG701 Hanoi – Sapa – Halong Bay – Da Nang',
        region: 'multi',
        category: 'grand-tour',
        excerpt: 'Featuring the two most popular and exciting travel destinations in Vietnam today - Experience the ultimate 7-day journey from Sapa\'s mountains through Halong Bay to Da Nang\'s coastal beauty.',
        description: 'VNG701 Hanoi – Sapa – Halong Bay – Da Nang showcases Vietnam\'s most iconic destinations in one comprehensive tour. Journey from the misty mountains of Sapa with signature Salmon Hotpot, explore the legendary Halong Bay, and discover Da Nang\'s coastal attractions including Ba Na Hills and Hoi An. This grand tour combines natural wonders, cultural heritage, and culinary excellence across four magnificent destinations.',
        image: {
            url: 'https://vn-to-you-tour.vercel.app/assets/images/tours/vng701.jpg',
            alt: 'VNG701 Hanoi Sapa Halong Bay Da Nang Grand Tour'
        },
        duration: {
            display: '7 Days / 6 Nights',
            days: 7,
            nights: 6
        },
        price: {
            display: 'Please contact us for pricing',
            amount: 0,
            currency: 'USD'
        },
        groupSize: 'Private tour for 2-10 people',
        highlights: [
            'Signature Sapa Dinner: Salmon Hotpot and Sashimi paired with Dalat Wine in Sapa\'s cool, misty air',
            'Fansipan Buffet offers a wide array of dishes, highlighting the diverse cuisine of Sapa',
            'A seafood hotpot featuring fresh Da Nang seafood and half a lobster for each guest',
            'Buffet at Ba Na Hills with stunning mountain views',
            'Visit Cat Cat Village and Sapa night market',
            'Conquer Fansipan peak with cable car and Muong Hoa Train',
            'Explore Mega Grand World Hanoi entertainment complex',
            'Boat trip through Halong Bay with Thien Cung Cave exploration',
            'Visit Hoi An Ancient Town with traditional basket boat experience',
            'Ba Na Hills full day: French Village, Moon Castle, Fantasy Park, Golden Hand Bridge',
            'Le Jardin D\'Amour Flower Garden in Ba Na Hills',
            'Multiple hotel tiers: 3*, 4*, and 5* options in all locations',
            '2 nights Sapa, 2 nights Hanoi, 1 night Halong Bay, 2 nights Da Nang'
        ],
        itinerary: [
            {
                day: 1,
                title: 'Arrival - Transfer to Sapa - Cat Cat Village',
                meals: 'Lunch, Dinner',
                activities: [
                    'Pick up at airport (or hotel)',
                    'Transfer to Sapa',
                    'Visit Cat Cat Village',
                    'Signature dinner: Salmon hotpot + Dalat wine',
                    'Shopping at night market Sapa',
                    'Overnight in Sapa'
                ],
                accommodation: 'Sapa Hotel (3*/4*/5* options available)'
            },
            {
                day: 2,
                title: 'Fansipan Peak Adventure',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Visit Fansipan (include Muong Hoa Train, Buffet Fansipan, Cable Car ticket)',
                    'Visit Moana Cafe (cafe fee not included)',
                    '<span style="color: red; font-weight: bold;">Optional Funicular Train Fansipan:</span>',
                    '<span style="color: red;">• $7 USD/pax up, $5 USD/pax down (Ideal for elderly guests who wish to skip the 600-step climb to the peak)</span>',
                    'Overnight in Sapa'
                ],
                accommodation: 'Sapa Hotel (3*/4*/5* options available)'
            },
            {
                day: 3,
                title: 'Sapa to Hanoi - Mega Grand World - Halong Bay',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Transfer to Hanoi',
                    'Visit Tran Quoc Pagoda',
                    'Visit Mega Grand World Hanoi',
                    'Transfer to Halong Bay',
                    'Dinner at Halong Bay',
                    'Overnight in Halong Bay'
                ],
                accommodation: 'Halong Bay Hotel'
            },
            {
                day: 4,
                title: 'Halong Bay Cruise - Return to Hanoi',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Boat trip through Halong Bay',
                    'Visit Thien Cung Cave (Heavenly Palace Cave)',
                    'Photo stop at Chicken Fighting Rock',
                    'Lunch on boat with fresh seafood',
                    'Return to Hanoi',
                    'Overnight in Hanoi'
                ],
                accommodation: 'Hanoi Hotel (3*/4*/5* options available)'
            },
            {
                day: 5,
                title: 'Flight to Da Nang - Hoi An Ancient Town',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Flight to Da Nang (airfare not included)',
                    'Transfer to Hoi An',
                    'Explore Hoi An Ancient Town',
                    'Traditional basket boat experience',
                    'Return to Da Nang',
                    'Overnight in Da Nang'
                ],
                accommodation: 'Da Nang Hotel (3*/4*/5* options available)'
            },
            {
                day: 6,
                title: 'Full Day Ba Na Hills Adventure',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Transfer to Ba Na Hills via cable car',
                    'Visit French Village',
                    'Moon Castle exploration',
                    'Fantasy Park experience',
                    'Visit Golden Hand Bridge',
                    'Le Jardin D\'Amour Flower Garden',
                    'Buffet lunch at Ba Na Hills',
                    'Return to Da Nang',
                    'Overnight in Da Nang'
                ],
                accommodation: 'Da Nang Hotel (3*/4*/5* options available)',
                notes: 'Ba Na Hills ticket not include Wax Museum fee ($4 USD/pax), Alpine Coaster ($4 USD/pax), Wine Cellar ($4 USD/pax)'
            },
            {
                day: 7,
                title: 'Da Nang City Tour - Departure',
                meals: 'Breakfast, Lunch',
                activities: [
                    'Breakfast at hotel',
                    'Last-minute shopping at Han Market',
                    'Transfer to airport',
                    'End of tour'
                ],
                accommodation: 'End of tour'
            }
        ],
        accommodations: {
            sapa: {
                '3star': [
                    'Azure Sapa 3*',
                    'Sapa Relax Hotel 3*',
                    'Or similar'
                ],
                '4star': [
                    'Sapa Highland Resort & Spa 4*',
                    'Charm Sapa Hotel 4*',
                    'Hotel De Sapa 4*',
                    'Or similar'
                ],
                '5star': [
                    'Available upon request'
                ]
            },
            hanoi: {
                '3star': [
                    'CWD Hanoi Hotel 3*',
                    'Quang Ba Trade Union Hotel 3*',
                    'Or similar'
                ],
                '4star': [
                    'Thang Long Opera Hotel 4*',
                    'Muong Thanh Central Hotel 4*',
                    'Super Candle Hanoi Hotel 4*',
                    'Lacasa Hanoi Hotel 4*',
                    'Or similar'
                ],
                '5star': [
                    'Available upon request'
                ]
            },
            danang: {
                '3star': [
                    'Sepon Blue Hotel 3*',
                    'Pandora Danang Hotel',
                    'Or similar'
                ],
                '4star': [
                    'Luxtery Da Nang 4*',
                    'Nhu Minh Danang 4*',
                    'Or similar'
                ],
                '5star': [
                    'Golden Bay Danang Hotel 5*',
                    'Muong Thanh Luxury Danang 5*',
                    'Or similar'
                ]
            }
        },
        meals: {
            total: 'Lunch = 7, Dinner = 6',
            specialties: [
                'Signature Sapa Dinner: Salmon Hotpot and Sashimi with Dalat Wine',
                'Fansipan Buffet with diverse Sapa cuisine',
                'Seafood hotpot with fresh Da Nang seafood and half lobster per guest',
                'Ba Na Hills Buffet with international cuisine',
                'Fresh seafood lunch on Halong Bay boat'
            ]
        },
        included: [
            'Accommodation twin share room (2 nights Sapa, 2 nights Hanoi, 1 night Halong Bay, 2 nights Da Nang)',
            'Meals: (B) Breakfast, (L) Lunch, (D) Dinner as specified',
            'Transportation with air-conditioning',
            'English / Chinese / Thai speaking guide',
            'Entrance fees to all attractions',
            'Drinking water during tours'
        ],
        excluded: [
            'Visa stamping fee in Vietnam',
            'Personal travel insurance',
            'Any flights to and from Vietnam (including Hanoi to Da Nang flight)',
            'All personal expenses',
            'All gratuities and tips to driver and tour guide',
            'Early check in & late check out at hotels',
            'Bank fees',
            'Surcharge on Christmas or New Year time',
            'Tips: Guide $2 USD/day, Driver $1 USD/day (Group <10 pax: Guide $4 USD/day, Driver $2 USD/day)'
        ],
        termsAndConditions: [
            'Rates are not applicable during: New Year 28 Dec - 2 Jan 2026 | Vietnamese New Year 15 - 20 Feb 2026 | 27 Apr 2026 | 30 Apr - 2 May 2026 | 1 - 3 Sept 2026 | 23 - 25 DEC 2026',
            'Tickets no refund in case of the tour does not visit various tourist attractions except in the event of a storm or flood',
            'When there is a force majeure that cannot be solved, the company has the right not to refund in all cases',
            'Cancellations 07 days before departure (or in the case of the company Deposit for hotel, car, etc.) will not return the money',
            'The company has the right not to be responsible for any expenses in the event of force majeure such as cancellation or delay of airlines / natural disasters / riots / political events and lost items at places where beyond the control of the company',
            'The travel program may be changed as appropriate without prior notice. The company will take into account the interests of customers as a priority'
        ],
        bestTime: 'Best time: March-May and September-November for ideal weather across all destinations. Avoid peak summer months for a more comfortable experience.',
        difficulty: 'Moderate - Suitable for most fitness levels with some walking and stair climbing at attractions',
        tags: ['vietnam-grand-tour', 'multi-region', 'sapa', 'hanoi', 'halong-bay', 'danang', 'hoian', 'fansipan', 'ba-na-hills', 'comprehensive', 'cultural-heritage', 'northern-central-vietnam'],
        featured: true,
        status: 'active',
        lastUpdated: '2025-12-11',
        seoTitle: 'VNG701 Hanoi Sapa Halong Bay Da Nang 7D6N Grand Tour | VN To You',
        seoDescription: 'Ultimate 7-day Vietnam Grand Tour covering four iconic destinations. Experience Sapa mountains, Halong Bay cruise, Da Nang coast with Ba Na Hills, and Hoi An Ancient Town. Includes signature Salmon Hotpot and seafood feast.',
        seoKeywords: 'Vietnam Grand Tour, Hanoi Sapa Halong Danang tour, 7 days Vietnam, Fansipan peak, Ba Na Hills, Golden Hand Bridge, Hoi An ancient town, Halong Bay, comprehensive Vietnam tour, multi-destination Vietnam'
    },
    {
        id: 'vng1001',
        title: 'VNG1001 Complete Vietnam Grand Tour: Hanoi – Sapa – Halong Bay – Da Nang – HCM – Muine',
        region: 'multi',
        category: 'grand-tour',
        excerpt: 'The most comprehensive Vietnam experience - 10 unforgettable days exploring from Northern mountains to Southern beaches, featuring all Vietnam\'s iconic destinations in one ultimate journey.',
        description: 'VNG1001 Complete Vietnam Grand Tour is our most extensive journey through Vietnam, covering the entire country from North to South. Experience Sapa\'s misty mountains, Halong Bay\'s emerald waters, Da Nang and Hoi An\'s cultural treasures, Ho Chi Minh City\'s vibrant energy, and Muine\'s stunning sand dunes. This 10-day adventure includes signature dining experiences: Salmon Hotpot in Sapa, seafood feast in Da Nang with half lobster per guest, dinner cruise on Saigon River, and BBQ in Dalat\'s romantic mountain atmosphere.',
        image: {
            url: 'https://vn-to-you-tour.vercel.app/assets/images/tours/vng1001.jpg',
            alt: 'VNG1001 Complete Vietnam Grand Tour - Hanoi Sapa Halong Da Nang HCM Muine'
        },
        duration: {
            display: '10 Days / 9 Nights',
            days: 10,
            nights: 9
        },
        price: {
            display: 'Please contact us for pricing',
            amount: 0,
            currency: 'USD'
        },
        groupSize: 'Private tour for 2-10 people',
        highlights: [
            'Signature Sapa Dinner: Salmon Hotpot and Sashimi paired with Dalat Wine in Sapa\'s cool, misty air',
            'Fansipan Buffet offers a wide array of dishes, highlighting the diverse cuisine of Sapa',
            'A seafood hotpot featuring fresh Da Nang seafood and half a lobster for each guest',
            'Enjoy a dinner cruise and admire the Saigon River by night',
            'A BBQ dinner in Dalat, enjoyed in the cool and romantic mountain atmosphere',
            'Buffet at Ba Na Hills with stunning views',
            'Complete Vietnam journey: North to South in 10 days',
            'Visit Cat Cat Village and Sapa night market',
            'Conquer Fansipan peak with cable car and Muong Hoa Train',
            'Halong Bay boat trip with Thien Cung Cave exploration',
            'Hoi An Ancient Town with traditional basket boat on Cam Thanh River',
            'Ba Na Hills: French Village, Moon Castle, Fantasy Park, Golden Hand Bridge, Flower Garden',
            'Muine White Sand Dunes adventure by Jeep',
            'Cu Chi Tunnels historical experience',
            'Saigon River dinner cruise',
            '2 nights Sapa, 2 nights Hanoi, 2 nights Da Nang, 1 night Hoi An, 3 nights HCM City'
        ],
        itinerary: [
            {
                day: 1,
                title: 'Arrival - Transfer to Sapa - Cat Cat Village',
                meals: 'Lunch, Dinner',
                activities: [
                    'Pick up at airport (or hotel)',
                    'Transfer to Sapa',
                    'Visit Cat Cat Village',
                    'Signature dinner: Salmon hotpot + Dalat wine',
                    'Shopping at night market Sapa',
                    'Overnight in Sapa'
                ],
                accommodation: 'Sapa Hotel (3*/4*/5* options available)'
            },
            {
                day: 2,
                title: 'Fansipan Peak Adventure',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Visit Fansipan (include Muong Hoa Train, Buffet Fansipan, Cable Car ticket)',
                    'Visit Moana Cafe (cafe fee not included)',
                    '<span style="color: red; font-weight: bold;">Optional Funicular Train Fansipan:</span>',
                    '<span style="color: red;">• $7 USD/pax up, $5 USD/pax down (Ideal for elderly guests who wish to skip the 600-step climb to the peak)</span>',
                    'Overnight in Sapa'
                ],
                accommodation: 'Sapa Hotel (3*/4*/5* options available)'
            },
            {
                day: 3,
                title: 'Sapa to Hanoi - City Tour',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Transfer back to Hanoi',
                    'Hanoi city tour',
                    'Visit Tran Quoc Pagoda',
                    'Hanoi Old Quarter exploration',
                    'Cafe Railway Hanoi (cafe fee not included)',
                    'Overnight in Hanoi'
                ],
                accommodation: 'Hanoi Hotel (3*/4*/5* options available)'
            },
            {
                day: 4,
                title: 'Halong Bay Day Trip - Return to Hanoi',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Transfer to Halong Bay',
                    'Boat trip through Halong Bay',
                    'Visit Thien Cung Cave (Heavenly Palace Cave)',
                    'Photo stop at Chicken Fighting Rock',
                    'Lunch on boat with fresh seafood',
                    'Return to Hanoi',
                    'Overnight in Hanoi'
                ],
                accommodation: 'Hanoi Hotel (3*/4*/5* options available)'
            },
            {
                day: 5,
                title: 'Flight to Da Nang - Hoi An Ancient Town',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Flight to Da Nang (airfare not included)',
                    'Transfer to Hoi An',
                    'Explore Hoi An Ancient Town',
                    'Traditional basket boat on Cam Thanh River',
                    'Overnight in Da Nang'
                ],
                accommodation: 'Da Nang Hotel (3*/4*/5* options available)'
            },
            {
                day: 6,
                title: 'Full Day Ba Na Hills Adventure',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Transfer to Ba Na Hills via cable car',
                    'Visit French Village',
                    'Moon Castle exploration',
                    'Fantasy Park experience',
                    'Visit Golden Hand Bridge',
                    'Le Jardin D\'Amour Flower Garden',
                    'Buffet lunch at Ba Na Hills',
                    'Return to Da Nang',
                    'Overnight in Da Nang'
                ],
                accommodation: 'Da Nang Hotel (3*/4*/5* options available)',
                notes: 'Ba Na Hills ticket not include Wax Museum fee ($4 USD/pax), Alpine Coaster ($4 USD/pax), Wine Cellar ($4 USD/pax)'
            },
            {
                day: 7,
                title: 'Da Nang - Flight to Ho Chi Minh City',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Visit Linh Ung Pagoda',
                    'Shopping at Han Market',
                    'Flight to Ho Chi Minh City (airfare not included)',
                    'Transfer to hotel',
                    'Overnight in Ho Chi Minh City'
                ],
                accommodation: 'HCM City Hotel (3*/4*/5* options available)'
            },
            {
                day: 8,
                title: 'Muine Day Trip - White Sand Dunes',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Transfer to Muine',
                    'Visit White Sand Dunes by Jeep adventure',
                    'Explore mystical Fairy Stream',
                    'Visit local Fishing Village',
                    'Return to Ho Chi Minh City',
                    'Overnight in Ho Chi Minh City'
                ],
                accommodation: 'HCM City Hotel (3*/4*/5* options available)'
            },
            {
                day: 9,
                title: 'Cu Chi Tunnels - Saigon River Cruise',
                meals: 'Breakfast, Lunch, Dinner',
                activities: [
                    'Breakfast at hotel',
                    'Half-day tour to Cu Chi Tunnels',
                    'Experience underground tunnel network',
                    'Learn about Vietnam War history',
                    'Return to city',
                    'Evening dinner cruise on Saigon River',
                    'Admire Ho Chi Minh City skyline by night',
                    'Overnight in Ho Chi Minh City'
                ],
                accommodation: 'HCM City Hotel (3*/4*/5* options available)'
            },
            {
                day: 10,
                title: 'Ho Chi Minh City Tour - Departure',
                meals: 'Breakfast, Lunch',
                activities: [
                    'Breakfast at hotel',
                    'Visit Independence Palace (Reunification Palace)',
                    'Explore unique Cafe Apartment',
                    'Walk along Nguyen Hue Walking Street',
                    'Last-minute shopping',
                    'Transfer to airport',
                    'End of tour'
                ],
                accommodation: 'End of tour'
            }
        ],
        accommodations: {
            sapa: {
                '3star': [
                    'Azure Sapa 3*',
                    'Sapa Relax Hotel 3*',
                    'Or similar'
                ],
                '4star': [
                    'Sapa Highland Resort & Spa 4*',
                    'Charm Sapa Hotel 4*',
                    'Hotel De Sapa 4*',
                    'Or similar'
                ],
                '5star': [
                    'Available upon request'
                ]
            },
            hanoi: {
                '3star': [
                    'CWD Hanoi Hotel 3*',
                    'Quang Ba Trade Union Hotel 3*',
                    'Or similar'
                ],
                '4star': [
                    'Thang Long Opera Hotel 4*',
                    'Muong Thanh Central Hotel 4*',
                    'Super Candle Hanoi Hotel 4*',
                    'Lacasa Hanoi Hotel 4*',
                    'Or similar'
                ],
                '5star': [
                    'Available upon request'
                ]
            },
            danang: {
                '3star': [
                    'Sepon Blue Hotel 3*',
                    'Pandora Danang Hotel',
                    'Or similar'
                ],
                '4star': [
                    'Luxtery Da Nang 4*',
                    'Nhu Minh Danang 4*',
                    'Or similar'
                ],
                '5star': [
                    'Golden Bay Danang Hotel 5*',
                    'Muong Thanh Luxury Danang 5*',
                    'Or similar'
                ]
            },
            hcmCity: {
                '3star': [
                    'Standard 3-star accommodations available'
                ],
                '4star': [
                    'Cicilia Saigon Hotel 4*',
                    'Athena Saigon Hotel 4*',
                    'Ramana Saigon Hotel 4*',
                    'Or similar'
                ],
                '5star': [
                    'Available upon request'
                ]
            }
        },
        meals: {
            total: 'Lunch = 10, Dinner = 9',
            specialties: [
                'Signature Sapa Dinner: Salmon Hotpot and Sashimi with Dalat Wine',
                'Fansipan Buffet with diverse Sapa cuisine',
                'Seafood hotpot with fresh Da Nang seafood and half lobster per guest',
                'Dinner cruise on Saigon River with city views',
                'BBQ dinner in Dalat with romantic mountain atmosphere',
                'Ba Na Hills Buffet with international cuisine',
                'Fresh seafood lunch on Halong Bay boat'
            ]
        },
        included: [
            'Accommodation twin share room (2 nights Sapa, 2 nights Hanoi, 2 nights Da Nang, 1 night Hoi An, 3 nights HCM City)',
            'Meals: (B) Breakfast, (L) Lunch, (D) Dinner as specified',
            'Transportation with air-conditioning',
            'English / Chinese / Thai speaking guide',
            'Entrance fees to all attractions',
            'Drinking water during tours'
        ],
        excluded: [
            'Visa stamping fee in Vietnam',
            'Personal travel insurance',
            'Any flights to and from Vietnam (including domestic flights: Hanoi-Da Nang, Da Nang-HCM)',
            'All personal expenses',
            'All gratuities and tips to driver and tour guide',
            'Early check in & late check out at hotels',
            'Bank fees',
            'Surcharge on Christmas or New Year time',
            'Tips: Guide $2 USD/day, Driver $1 USD/day (Group <10 pax: Guide $4 USD/day, Driver $2 USD/day)'
        ],
        termsAndConditions: [
            'Rates are not applicable during: New Year 28 Dec - 2 Jan 2026 | Vietnamese New Year 15 - 20 Feb 2026 | 27 Apr 2026 | 30 Apr - 2 May 2026 | 1 - 3 Sept 2026 | 23 - 25 DEC 2026',
            'Tickets no refund in case of the tour does not visit various tourist attractions except in the event of a storm or flood',
            'When there is a force majeure that cannot be solved, the company has the right not to refund in all cases',
            'Cancellations 07 days before departure (or in the case of the company Deposit for hotel, car, etc.) will not return the money',
            'The company has the right not to be responsible for any expenses in the event of force majeure such as cancellation or delay of airlines / natural disasters / riots / political events and lost items at places where beyond the control of the company',
            'The travel program may be changed as appropriate without prior notice. The company will take into account the interests of customers as a priority'
        ],
        bestTime: 'Best time: March-May and September-November for ideal weather across all regions from North to South. This comprehensive tour showcases Vietnam in its full glory.',
        difficulty: 'Moderate - Suitable for most fitness levels with some walking and stair climbing. A good fitness level is recommended for this extensive tour.',
        tags: ['vietnam-grand-tour', 'complete-vietnam', 'multi-region', 'sapa', 'hanoi', 'halong-bay', 'danang', 'hoian', 'hcm-city', 'muine', 'fansipan', 'ba-na-hills', 'cu-chi-tunnels', 'comprehensive', 'ultimate-tour', 'north-to-south'],
        featured: true,
        status: 'active',
        lastUpdated: '2025-12-11',
        seoTitle: 'VNG1001 Complete Vietnam Grand Tour 10D9N - Hanoi Sapa Halong Da Nang HCM Muine | VN To You',
        seoDescription: 'The ultimate 10-day Vietnam Grand Tour from North to South. Experience Sapa mountains, Halong Bay, Da Nang coast, Hoi An heritage, HCM City energy, and Muine sand dunes. Includes Salmon Hotpot, seafood feast, Saigon River cruise, and complete Vietnam discovery.',
        seoKeywords: 'Complete Vietnam tour, 10 days Vietnam, Vietnam Grand Tour, North to South Vietnam, Sapa Halong Danang HCM, Fansipan peak, Ba Na Hills, Cu Chi Tunnels, Muine sand dunes, comprehensive Vietnam travel, ultimate Vietnam tour, multi-destination Vietnam'
    }
];
